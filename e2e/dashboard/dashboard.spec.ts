// SCR-DASHBOARD 대시보드 — FSD §7 예외 처리 E2E (담당: E2E 테스트 — e2e/**)
//
// 명세: docs/FSD/dashboard/index.md  ·  구현: apps/admin/src/pages/dashboard/** + shared/layout/**
// 이름 규칙: `<화면 ID> / <§7 예외 상황> — <단언>` (tools/test-coverage 축 4의 대조 키)
//
// [예외 상황의 주입 지점]
...
// 통계 조회(stats-api.ts)에는 재현 파라미터가 **없다** — 그래서 통계의 조회 실패·0건 상황은 덮지 못했다.
...
// [인증 전제] 셸 라우트는 세션이 없으면 /login 으로 보낸다. 인증 자체는 SCR-LOGIN 이 소유한다.
...
test('SCR-DASHBOARD / API 오류 — 탭 데이터 조회가 실패하면 패널 대신 배너를 렌더하고 통계는 영향을 받지 않는다', async ({
  page,
}) => {
  await page.goto('/dashboard?error=1');

  // 인라인 배너다 — 토스트가 아니다 (사라지면 안 되는 안내다)
  const banner = page.getByRole('alert');
  await expect(banner).toHaveText(
    '대시보드 데이터를 불러오지 못했습니다. 잠시 후 다시 시도해 주세요.',
  );

  // 패널(할일 + 리스트 카드 2장) 전체가 렌더되지 않는다
...
test('SCR-DASHBOARD / 권한 없음 — 모든 메뉴 권한이 꺼지면 내비게이션이 비고 로고 칸만 남는다', async ({
  page,
}) => {
...
test('SCR-DASHBOARD — 조회 중에는 카드가 aria-busy 로 남고 탭바는 잠기지 않는다', async ({
  page,
}) => {
  await page.goto('/dashboard?delay=2000');

  // 할일 카드는 골격을 유지한 채 aria-busy 로 스켈레톤을 보인다
  const todo = cardByTitle(page, '오늘의 할일');
  await expect(todo).toHaveAttribute('aria-busy', 'true');

  // 최초 조회에서는 자리표시 카드 2장을 미리 렌더해 레이아웃 높이를 지킨다
  ...
  // 탭바는 잠기지 않는다 — 조회 중에도 다른 탭을 즉시 선택할 수 있다
...
test('SCR-DASHBOARD — 통계 조회 중에는 기간 토글 세 개가 모두 비활성되고 두 카드가 aria-busy 로 남는다', async ({
  page,
}) => {
  await page.goto('/dashboard');
...
test('SCR-DASHBOARD — 탭을 전환하면 늦게 도착한 이전 탭의 응답이 새 탭 데이터를 덮어쓰지 않는다', async ({
  page,
}) => {
...
test('SCR-DASHBOARD — 다른 탭에서 권한을 바꾸면 리로드 없이 즉시 반영된다', async ({ page }) => {
  await page.goto('/dashboard');
  await expect(page.getByRole('heading', { name: '오늘의 할일' })).toBeVisible();
...
test('SCR-DASHBOARD — 등록되지 않은 키는 버리고 빠진 키는 기본값 ON 으로 채운다', async ({
  page,
}) => {
  await seedRoleState(
    page,
...
test('SCR-DASHBOARD / 권한 없음 — dashboard.todo 가 꺼지면 오늘의 할일 카드가 통째로 사라진다 (비활성이 아니라 미렌더)', async ({
  page,
}) => {
...
test('SCR-DASHBOARD / 권한 없음 — 모든 위젯이 꺼지면 본문이 안내 문구 한 줄로 대체되고 셸은 유지된다', async ({
  page,
}) => {
...
test('SCR-DASHBOARD / 권한 없음 — 보던 탭의 권한이 꺼지면 남은 첫 탭으로 활성 탭이 옮겨간다', async ({
  page,
}) => {
 ...
 *
 * SCR-DASHBOARD(권한없음): "`dashboard.tab.products` 가 꺼지면 **이 탭이 사라진다**".
 * SCR-DASHBOARD(권한없음): "꺼진 탭은 목록에서 사라지고, **남은 탭만 렌더된다**".
 *
 ...
 */
test('SCR-DASHBOARD / 권한 없음 — 권한이 꺼진 탭은 탭바에서 사라지고 남은 탭만 렌더된다', async ({
  page,
}) => {
  await seedPermissions(page, {
    'dashboard.tab.products': false,
 ...
 */
test('SCR-DASHBOARD / 권한 없음 — 권한이 꺼진 메뉴는 사이드바에서 사라지고, 그 화면을 deep-link 하면 403 이 뜬다', async ({
  page,
}) => {
  await seedPermissions(page, { 'menu.dashboard': false, 'menu.users': false });
  await page.goto('/dashboard');

  // 잎이 사라진다 / 가지가 사라진다 / 서브메뉴도 함께
...
test('SCR-DASHBOARD / 권한 없음 — 모든 메뉴 권한이 꺼져도 로고·헤더는 남고 항목이 0개인 섹션 제목은 사라진다', async ({
  page,
}) => {
  await seedPermissions(page, allMenusOff);
  await page.goto('/dashboard');

  // 표시 가능한 항목이 0개인 섹션은 제목까지 렌더하지 않는다
  ...
  await expect(page.getByText('test@example.com')).toBeVisible(); // EL-011
});

test('SCR-DASHBOARD / 권한 없음 — 이동 대상 화면의 메뉴 권한이 꺼져 있어도 카드 안의 링크는 그대로 동작한다', async ({
  page,
}) => {
...
test('SCR-DASHBOARD — 사이드바는 섹션 5개와 확장형 가지 12개를 고정으로 렌더한다', async ({
  page,
}) => {
  await page.goto('/dashboard');

  // 섹션 5개: 일반 관리 · 비즈니스 · AI · 분석 · 운영 · 시스템 (nav-config.ts NAV_SECTIONS)
  await expect(nav(page).getByRole('heading')).toHaveCount(5);
  // 가지만 button 이다 (잎은 link) — 12개.
  // 대시보드는 가지가 아니라 최상위 잎이라 여기에 세지 않는다.
  await expect(nav(page).getByRole('button')).toHaveCount(12);
});

test('SCR-DASHBOARD — 기간을 바꾸면 데이터 점 수와 x축 라벨 수가 그에 맞게 바뀐다', async ({
  page,
}) => {
  await page.goto('/dashboard');
...
test('SCR-DASHBOARD — 기간별 분석 표는 합계 행 2건을 전부 렌더한다', async ({ page }) => {
  await page.goto('/dashboard');
