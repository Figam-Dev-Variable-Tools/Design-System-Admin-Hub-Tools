// FS-002 대시보드 — 예외 7축 E2E (A85 소유 — e2e/**)
//
...
// (A40 에게 `?statsError=1` · `?statsEmpty=1` · `?statsDelay=<ms>` 를 요청한다 — 보고서 참조)
...
/**
 * ⛔ 이 테스트는 **실패한다 — 구현 결함이다** (A40 change_request).
 *
...
test('FS-002-EL-007 / FS-002-EL-003 / FS-002-EL-004 / FS-002-EL-005: 권한없음 — 권한이 꺼진 메뉴는 사이드바에서 사라지지만 라우트 접근 자체는 차단되지 않는다', async ({
  page,
}) => {
  ...
  // FS-002-EL-003: **메뉴가 사라져도 라우트는 차단되지 않는다** — 명세가 그렇게 정했다.
  // (프론트 권한은 보안 경계가 아니다. 실제 차단은 서버의 몫이다 — 보고서 §권한 축 참조)
