// SCR-USERS-MEMBERS-DETAIL 회원 상세 — FSD §7 예외 처리 E2E (담당: E2E 테스트 — e2e/**)
//
// 명세: docs/FSD/users/members/detail.md
...
//
// [적립금은 돈이다]
// 적립금 지급은 **비멱등**이며 `Idempotency-Key` 헤더가 필수다. 같은 키 + 같은 바디는
// 서버가 최초 응답을 재생한다(중복 지급 없음). 적발됐던 결함은 프론트가 **호출마다**
...
//
// [명세 정합] 내역 삭제에는 구현이 확인 다이얼로그를 세워 두었고, 실패 안내도 드러나 있다.
// 명세 §7 의 `저장 실패` · `삭제 실패` 행이 그 문구를 그대로 인용한다.
...
test('SCR-USERS-MEMBERS-DETAIL — 상세 조회 중에는 본문이 스켈레톤으로 대체되고 액션 메뉴는 렌더되지 않는다', async ({
  page,
}) => {
  ...
  // 회원이 특정되지 않았으므로 삭제·발송 대상이 없다 — 메뉴를 띄우지 않는다
  await expect(u.actions).toHaveCount(0);
  // 돌아가기 링크는 로딩 중에도 살아 있다
...
test("SCR-USERS-MEMBERS-DETAIL — 지급 요청 중에는 확인 버튼이 '처리 중…' 으로 잠긴다", async ({
  page,
}) => {
...
test('SCR-USERS-MEMBERS-DETAIL / 저장 실패 — 지급이 실패하면 잔액을 건드리지 않고 안내를 띄우며, 재시도는 같은 멱등키로 처리되어 두 번 지급되지 않는다', async ({
  page,
}) => {
...
test('SCR-USERS-MEMBERS-DETAIL / 삭제 실패 — 내역 삭제가 실패하면 다이얼로그 안에서 알리고 행과 잔액을 그대로 둔다', async ({
  page,
}) => {
...
test('SCR-USERS-MEMBERS-DETAIL / API 오류 — 상세 조회가 실패하면 카드 대신 배너를 띄우고 돌아가기 링크는 남는다', async ({
  page,
}) => {
...
test('SCR-USERS-MEMBERS-DETAIL — 알림 발송이 실패하면 조용히 삼키지 않고 실패를 알리고 재시도 경로를 준다', async ({
  page,
}) => {
...
test('SCR-USERS-MEMBERS-DETAIL / 삭제 실패 — 회원 삭제가 실패하면 다이얼로그가 닫히지 않고 화면을 떠나지 않는다', async ({
  page,
}) => {
...
test('SCR-USERS-MEMBERS-DETAIL / 조회 결과 없음 — 존재하지 않는 회원 id 로 들어가면 회원을 찾을 수 없다고 알린다', async ({
  page,
}) => {
...
test('SCR-USERS-MEMBERS-DETAIL — 금액·사유·차감 상한을 검증하고 위반이면 서버를 호출하지 않는다', async ({
  page,
}) => {
  await page.goto(MEMBER);
  const u = ui(page);
...
test('SCR-USERS-MEMBERS-DETAIL — 제출 중 이중 클릭이 적립금을 두 번 올리지 않는다', async ({
  page,
}) => {
  await spyIdempotencyKeys(page);
  await page.goto(MEMBER);
  ...
  // 잔액은 한 번만 움직인다 (12,000 이면 이중 지급이다)
  await expect(u.balance).toHaveText('11,000 포인트 (KRW)');
  // 내역 행도 1건만 늘어난다
  await expect(u.historyRows).toHaveCount(before + 1);
