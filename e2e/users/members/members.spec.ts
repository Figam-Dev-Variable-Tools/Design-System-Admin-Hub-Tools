// SCR-USERS-MEMBERS 회원 목록 — FSD §7 예외 처리 E2E (담당: E2E 테스트 — e2e/**)
//
// 명세: docs/FSD/users/members/index.md  ·  구현: apps/admin/src/pages/members/**
// 이름 규칙: `<화면 ID> / <§7 예외 상황> — <단언>` (tools/test-coverage 축 4의 대조 키)
//
// [예외 상황의 주입 지점] data-source.ts 의 실패 스위치 — `?fail=members,groups,export,notify,delete,...`
// 지연(400ms)·빈 상태에는 파라미터가 없다: 빈 상태는 **검색으로** 만든다(그것이 실제 사용자 경로다).
//
// [명세 정합] 알림 발송 실패 · 삭제 실패의 안내는 구현에 이미 있고, 명세 §7 의 `삭제 실패` 행이
// 그 문구를 그대로 인용한다. 아래 테스트가 그 동작의 회귀 방어선이다.
...
// (인증 자체는 SCR-LOGIN 이 소유한다 — 여기서 다시 검증하지 않는다.)
 ...
 * 시작될 때 스켈레톤 창은 이미 닫혀 있다. 회원 상세의 로딩 테스트가 3회 중 1회 그렇게 터졌고,
...
test('SCR-USERS-MEMBERS / 조회 결과 없음 — 검색 결과가 0건이면 표 본문이 빈 상태 문구로 바뀌고 페이지네이션이 사라진다', async ({
  page,
}) => {
  ...
  // 표 본문 전체를 병합한 셀에 문구를 보여준다
  await expect(page.getByText('검색 결과가 없습니다.')).toBeVisible();
  // 본문이 빈 상태로 대체된다 (행 1줄만 남는다)
  await expect(u.rows).toHaveCount(1);
  await expect(page.getByText('전체 0명')).toBeVisible();
  // 총 페이지가 1 이하면 페이지네이션 영역 자체가 렌더되지 않는다
  await expect(u.pagination).toHaveCount(0);
});

/* ── 로딩 ────────────────────────────────────────────────────────────────── */

test('SCR-USERS-MEMBERS — 조회 중에는 표가 aria-busy 로 스켈레톤 10행을 보이고 요약이 불러오는 중으로 바뀐다', async ({
  page,
}) => {
  ...
  await expect(u.table).toHaveAttribute('aria-busy', 'true'); // SCR-USERS-MEMBERS
  await expect(page.getByText('불러오는 중…')).toBeVisible(); // SCR-USERS-MEMBERS
  // 본문은 페이지 크기(10행) 만큼의 스켈레톤이다 — 아직 회원 링크가 없다
  await expect(u.rows).toHaveCount(10);
...
test('SCR-USERS-MEMBERS — 조회 중 키워드가 바뀌면 마지막 키워드만 반영된다', async ({ page }) => {
  await page.goto(LIST);
  const u = ui(page);
  await expect(u.rows).toHaveCount(10);
...
test('SCR-USERS-MEMBERS — 내보내는 중에는 버튼 라벨이 바뀌고 비활성되어 다시 눌리지 않는다', async ({
  page,
}) => {
  await page.goto(LIST);
  const u = ui(page);
...
test('SCR-USERS-MEMBERS / API 오류 — 목록 조회가 실패하면 표·요약·페이지네이션이 배너로 대체되고 재시도가 다시 실패하면 배너가 다시 뜬다', async ({
  page,
}) => {
  ...
  // 마지막 조회 조건을 그대로 재조회한다 — 다시 실패하면 같은 배너가 다시 뜬다
  await page.getByRole('button', { name: '다시 시도' }).click();
  await expect(banner).toContainText('회원 목록을 불러오지 못했습니다.');
});

test('SCR-USERS-MEMBERS — 알림 발송이 실패하면 조용히 삼키지 않고 실패를 알리고 재시도 경로를 준다', async ({
  page,
}) => {
...
test('SCR-USERS-MEMBERS / 삭제 실패 — 삭제가 실패하면 다이얼로그가 닫히지 않고 실패 안내를 띄운다', async ({
  page,
}) => {
...
test('SCR-USERS-MEMBERS — 행 액션 메뉴의 두 항목은 항상 표시되며 비활성되지 않는다', async ({
  page,
}) => {
  await page.goto(LIST);
  await expect(ui(page).rows).toHaveCount(10);
...
test('SCR-USERS-MEMBERS — 497명을 페이지당 10행으로 나누고 번호 버튼은 최대 5개만 노출한다', async ({
  page,
}) => {
  await page.goto(LIST);
  const u = ui(page);
...
test('SCR-USERS-MEMBERS — 행의 빈 영역을 누르면 상세로 가지만, 행 안의 체크박스를 누르면 이동하지 않는다', async ({
  page,
}) => {
  await page.goto(LIST);
  const u = ui(page);
  ...
  // 체크박스는 자기 일만 한다 — 여기서 화면이 튀면 목록에서 아무것도 고를 수 없다
...
test('SCR-USERS-MEMBERS — 회원은 확인 다이얼로그를 거치지 않고 삭제되지 않는다', async ({
  page,
}) => {
  await page.goto(LIST);
  const u = ui(page);
