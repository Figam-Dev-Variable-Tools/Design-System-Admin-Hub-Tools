// 모션 — quality-bar MOTION 축 E2E (e2e/**)
//
// 명세: docs/reference/quality-bar.md §MOTION (MOTION-01/02/03/08/09)
// 구현: packages/ui/src/organisms/Modal · molecules/Toast · atoms/ToggleSwitch
//
// [왜 화면 ID 가 없나] 다른 e2e 파일은 화면(FSD) 하나를 미러링하고 이름에 그 화면 ID 를 박는다.
// MOTION 은 특정 화면이 아니라 **모든 화면을 가로지르는 품질 기준**이라 화면 ID 가 없다.
// 그래서 tools/test-coverage 축 4의 격자에도 오르지 않는다 — 축 4는 화면별 §7 만 잰다.
//
