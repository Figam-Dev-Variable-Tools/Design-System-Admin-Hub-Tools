...
**이 YAML 은 원천이 아니다.** 원천은 BE 명세(A63 소유)다.
...
명세를 바꿔야 한다면 그것은 **A63 의 일**이며, YAML 을 먼저 고쳐서 명세를 따라오게 만들지 않는다.

```
 FS 명세 (A62)  →  BE 명세 (A63)  →  openapi.yaml  →  schema.d.ts  →  프론트 타입 검증
...
## 4. 미확정 항목 (A01 판정 대상 — 이 스키마가 정하지 않는다)

| # | 항목 | 현재 스키마의 처리 |
|---|---|---|
| 1 | **세션 자격 증명의 전달 방식** (HttpOnly 쿠키 vs `Authorization` 헤더) | BE-001 §7.1 이 명시적으로 **확정을 보류**했다. `securitySchemes.sessionAuth` 를 쿠키로 표기했으나, **401 판정은 전달 방식과 무관하게 '유효한 자격 증명 없음'으로 성립한다**. A01 이 판정하면 그 블록만 고친다 — **엔드포인트 계약은 바뀌지 않는다** |
| 2 | **역할 어휘 불일치** | BE-001/002 는 `system_admin`·`operator`·`viewer`, BE-003/004 는 `admin`·`operator` 다. 스키마의 `UserRole` 은 **프론트 타입과 일치하는 BE-001 쪽**을 따랐다. `admin` = `system_admin` 인지 명세에 없다 → **A01/A63** |
| 3 | **에러 봉투의 `details` 필드** | BE-001/002 는 **필수**(`WIDGET_FORBIDDEN` 의 `requiredKeys` 등이 여기 실린다), BE-003/004 §2 봉투 표에는 **행이 없다**. 스키마는 **선택(nullable)** 으로 모델링해 양쪽을 만족시켰다 → **A63** |
| 4 | **권한 조회 엔드포인트** | **만들지 않았다.** FS-002 §5 에 서버 연동 요소가 없다 (BE-002 §7.1 — A62 change_request 대기). 근거 FS 요소가 없는 엔드포인트는 **유령 기능**이 된다 |

## 5. 이 스키마를 고쳐야 할 때

1. **BE 명세를 먼저 읽는다.** 명세에 없는 것을 스키마에 넣지 않는다 (유령 엔드포인트).
2. 명세가 틀렸다면 **A63 에 change_request** 를 보낸다. 스키마를 먼저 고치지 않는다.
