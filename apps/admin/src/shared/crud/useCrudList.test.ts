...
describe('bulkDeleteErrorMessage — 일괄 삭제 실패 문구', () => {
  it('409 사유가 있으면 그 문장을 그대로 보여 주고 재시도를 권하지 않는다', () => {
    const message = bulkDeleteErrorMessage(3, [
      conflict('발송 규칙 3건이 이 템플릿을 쓰고 있어 삭제할 수 없어요.'),
    ]);

    expect(message).toContain('3건 중 1건을 삭제하지 못했어요.');
    expect(message).toContain('발송 규칙 3건이 이 템플릿을 쓰고 있어 삭제할 수 없어요.');
  ...
  it('409 가 아닌 실패만 있으면 재시도를 권한다 (500 은 실제로 시간이 푼다)', () => {
    ...
    expect(message).toBe('2건 중 1건을 삭제하지 못했어요. 잠시 후 다시 시도해 주세요.');
  });

  it('409 와 재시도 가능 실패가 섞이면 둘 다 말한다', () => {
    const message = bulkDeleteErrorMessage(5, [conflict('참조가 남아 있어요.'), serverError()]);

    expect(message).toContain('5건 중 2건을 삭제하지 못했어요.');
    expect(message).toContain('참조가 남아 있어요.');
    // 한쪽만 고쳐서는 끝낼 수 없으므로 재시도 안내도 남는다
    expect(message).toContain('나머지는 잠시 후 다시 시도해 주세요.');
  });

  it('같은 409 사유가 여러 건이어도 문장은 한 번만 (소음 방지)', () => {
    const reason = '참조가 남아 있어 삭제할 수 없어요.';
    const message = bulkDeleteErrorMessage(3, [
      conflict(reason),
      conflict(reason),
      conflict(reason),
    ...
    expect(message).toBe(`3건 중 3건을 삭제하지 못했어요. ${reason}`);
  ...
  it('서로 다른 409 사유는 모두 보여 준다', () => {
    ...
    expect(message).toContain('규칙이 써요.');
    expect(message).toContain('이미 삭제됐어요.');
  });
  ...
  it('메시지 없는 409 는 재시도 안내로 떨어진다 (빈 문장을 보여 주지 않는다)', () => {
    ...
    expect(message).toBe('1건 중 1건을 삭제하지 못했어요. 잠시 후 다시 시도해 주세요.');
  });
