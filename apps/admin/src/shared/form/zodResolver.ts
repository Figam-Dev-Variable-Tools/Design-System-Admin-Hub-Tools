// react-hook-form ↔ zod 연결 (A41 — ADR-0008 §7.2 · §7.3 집행)
//
// [왜 손으로 썼는가 — A80 에 change_request 대상]
// 정식 어댑터는 `@hookform/resolvers/zod` 다. 그러나 ADR-0008 §3.3 이 그 패키지를
// **의도적으로 설치하지 않았고**("A41 이 zod 결합을 실제로 쓸 때 요청한다"),
// `package.json` 은 A80 소유라 A41 이 `pnpm add` 를 실행할 수 없다.
//
...
//
// → **A80 handoff**: `@hookform/resolvers` 설치 후 이 파일을 지우고 `zodResolver` 로 교체한다.
