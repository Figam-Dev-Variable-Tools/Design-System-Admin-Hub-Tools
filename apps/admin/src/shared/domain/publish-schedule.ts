...
//
// ─────────────────────────────────────────────────────────────────────────────
// [왜 페이지 밖에 있나] 발행을 말하는 화면이 둘이다.
//   · 페이지 관리(/content/pages) — 임의 페이지의 발행/예약.
//   · 뉴스·보도자료(/content/news) — 게시판 글의 발행/예약.
// 둘이 각자 '예약' 을 판정하면 같은 시각에 한쪽은 '예약', 다른쪽은 '발행' 이라고 말하는 날이 온다.
// 규칙이 두 벌이면 언제나 갈라진다 — 그래서 판정은 여기 한 곳에만 있다.
//
...
export const PUBLISH_AT_INVALID = '공개 일시 형식이 올바르지 않습니다.';
export const PUBLISH_AT_NOT_PUBLISHED =
  '공개 일시는 발행 상태에서만 지정할 수 있습니다. 초안·보관은 예약되지 않습니다.';
