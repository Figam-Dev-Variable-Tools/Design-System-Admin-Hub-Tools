// 다중 이미지 갤러리 필드 회귀 테스트 (A41) — 다중 선택·개별 제거·개수 상한·빈 placeholder
...
describe('ImageGalleryField', () => {
  ...
  it('개별 제거는 그 항목만 뺀 배열로 onChange 한다', () => {
  ...
  it('개수 상한을 넘기면 상한까지만 담고 오류를 알린다', () => {
    let captured: readonly string[] = [];
    const onChange = vi.fn((next: readonly string[]) => {
      captured = next;
    });
