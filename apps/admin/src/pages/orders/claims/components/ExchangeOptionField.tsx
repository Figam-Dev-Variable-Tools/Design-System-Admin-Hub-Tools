// 교환 옵션(SKU) 선택 + 재고 이동 미리보기 (클레임 상세 1곳 전용)
//
// 옵션 매트릭스의 정본은 상품 관리다 — 여기서 조합을 다시 만들지 않고 조회기가 준 목록
// (shared/domain/variant-ref)을 그대로 선택지로 쓴다. 재고가 수량보다 적은 조합은 고를 수 없게
// 막고(유효성), 고르면 완료 시 어떤 재고가 어떻게 움직이는지 먼저 보여 준다 — 되돌릴 수 없는
// 작업이라 예고한다.
//
...
interface ExchangeOptionFieldProps {
  readonly variants: readonly VariantRef[];
  /** 주문된(회수될) 옵션 조합 */
    ...
    <>
      ...
      >
        <SelectField
          ...
          aria-describedby={invalid ? errorIdOf('claim-exchange-option') : undefined}
          onChange={(event) => {
