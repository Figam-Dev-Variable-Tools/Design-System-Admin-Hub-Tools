// 프로그램 카테고리 목록의 행 액션 — 연필과 휴지통은 각각 update/remove 를 탄다 (EXC-03)
//
// ─────────────────────────────────────────────────────────────────────────────
// [무엇을 지키나]
// 이 목록은 2단계 트리라 표가 아니고, 그래서 `CrudListShell` 을 쓰지 않는다. 껍데기가 대신
// 해 주던 행 액션 게이팅(canUpdate/canRemove)이 여기엔 없었고, **등록만 걸린 채 수정·삭제는
// 조회 권한만으로 열려 있었다.** 상품 카테고리와 **같은 파일 구조의 복제본**이라 같은 구멍이
// 같은 자리에 있었다 — 그래서 단언도 같은 모양으로 둔다. 한쪽만 고치면 다음에 복제될 때 다시
// 열린 쪽이 정본이 된다.
//
...
import ProgramCategoriesPage from './ProgramCategoriesPage';

const ROUTE = '/programs/categories';
    ...
    <QueryClientProvider client={client}>
      <ToastProvider>
        <MemoryRouter initialEntries={[ROUTE]}>
          <Routes>
            <Route path={ROUTE} element={<ProgramCategoriesPage />} />
          </Routes>
        </MemoryRouter>
      </ToastProvider>
    </QueryClientProvider>,
 ...
 * 시드의 대분류 이름을 탐침으로 쓴다(`programs/_shared/store` 의 카테고리 시드).
 */
async function waitForRows(): Promise<void> {
  await waitFor(() => {
    expect(screen.getByText('테크·가전')).not.toBeNull();
  });
}
...
describe('ProgramCategoriesPage — 행 액션은 각자의 권한을 탄다 (EXC-03)', () => {
