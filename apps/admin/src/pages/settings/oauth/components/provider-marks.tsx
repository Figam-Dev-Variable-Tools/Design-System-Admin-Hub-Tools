 ...
 *
 * ┌ viewBox 가 `0 0 48 48` 이 아닌 이유 — 광학 정렬 ─────────────────────────┐
 * │ 이 마크만 **낱개 글리프**다. 나머지 다섯(카카오·네이버·페이스북·애플·라인)은  │
 * │ 자기 앱 아이콘의 라운드 사각 판까지 포함한 도형이라 상자를 가득 채운다.       │
 * │                                                                          │
 * │ 원본 자산의 아트워크는 x 2~45.12 · y 2~46 구간에 있다 — 즉 48 짜리 상자 안에서 │
 * │ ① 상자의 92% 만 차지하고 ② 가로 중심이 23.56 이라 **0.44 만큼 왼쪽으로 치우쳐**│
 * │ 있다. 판을 두른 다섯 옆에 세우면 혼자 작고 왼쪽으로 밀려 보인다.             │
 * │                                                                          │
 * │ 그래서 **아트워크의 실제 경계에 맞춰 정사각으로 잘라 낸다** — 중심 (23.56, 24)│
 * │ 를 상자 중심으로 삼고 한 변을 44(높이)로 둔다. path 는 한 글자도 고치지 않았다:│
 * │ 자산은 그대로고 **바라보는 창**만 옮겼다. 로고를 다시 그리는 것이 아니므로     │
 * │ '없는 로고를 지어내지 않는다' 는 이 파일의 금지와 충돌하지 않는다.            │
 * └──────────────────────────────────────────────────────────────────────────┘
 */
function GoogleMark() {
  return (
    <svg {...MARK_BASE} viewBox="1.56 2 44 44">
      <path
