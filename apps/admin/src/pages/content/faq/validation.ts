...
export const faqSchema = z.object({
  question: z.string().check(
    ...
    z.refine((value) => value.trim().length <= QUESTION_MAX_LENGTH, {
      error: `질문은 ${String(QUESTION_MAX_LENGTH)}자를 넘을 수 없어요.`,
    }),
  ),
  categoryId: z
    .string()
  ...
  answer: z.string().check(
    ...
    z.refine((value) => value.length <= ANSWER_MAX_LENGTH, {
      error: `답변은 ${String(ANSWER_MAX_LENGTH)}자를 넘을 수 없어요.`,
    }),
  ),
  visible: z.boolean(),
  ...
  order: z.string().check(
    ...
    z.refine((value) => /^\d+$/.test(value.trim()), { error: '정렬 순서는 0 이상의 정수예요.' }),
  ),
});
...
export const faqCategorySchema = z.object({
  name: z.string().check(
    ...
    z.refine((value) => value.trim().length <= CATEGORY_NAME_MAX_LENGTH, {
      error: `카테고리명은 ${String(CATEGORY_NAME_MAX_LENGTH)}자를 넘을 수 없어요.`,
    }),
  ),
});
