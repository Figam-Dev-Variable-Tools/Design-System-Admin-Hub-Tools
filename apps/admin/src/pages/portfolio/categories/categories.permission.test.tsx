// 포트폴리오 분류 목록의 행 액션 — 연필과 휴지통은 각각 update/remove 를 탄다 (EXC-03)
//
// ─────────────────────────────────────────────────────────────────────────────
// [무엇을 지키나]
...
import PortfolioCategoriesPage from './PortfolioCategoriesPage';

const ROUTE = '/portfolio/categories';
    ...
    <QueryClientProvider client={client}>
      <ToastProvider>
        <MemoryRouter initialEntries={[ROUTE]}>
          <Routes>
            <Route path={ROUTE} element={<PortfolioCategoriesPage />} />
          </Routes>
        </MemoryRouter>
      </ToastProvider>
    </QueryClientProvider>,
...
async function waitForRows(): Promise<void> {
  await waitFor(() => {
    expect(screen.getByText('주거 공간')).not.toBeNull();
  });
}
...
describe('PortfolioCategoriesPage — 행 액션은 각자의 권한을 탄다 (EXC-03)', () => {
