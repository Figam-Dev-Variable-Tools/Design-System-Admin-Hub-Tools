// 문의 유형 목록의 행 액션 — 연필과 휴지통은 각각 update/remove 를 탄다 (EXC-03)
//
// ─────────────────────────────────────────────────────────────────────────────
// [무엇을 지키나]
// 이 목록은 표가 아니라 <ul> 이라 `CrudListShell` 을 쓰지 않는다. 껍데기가 대신 해 주던 행 액션
// 게이팅(canUpdate/canRemove)이 여기엔 없었고, **등록·수정·삭제가 전부 조회 권한만으로 열려
// 있었다.** 껍데기를 쓰지 않는 목록이 게이팅에서 통째로 빠지는 이 모양은 상품·프로그램 카테고리
// 에도 같은 자리에 있었다(서로 복제본이다).
//
...
const ROUTE = '/support/categories';
    ...
    <QueryClientProvider client={client}>
      <ToastProvider>
        <MemoryRouter initialEntries={[ROUTE]}>
          <Routes>
            <Route path={ROUTE} element={<CategoriesPage />} />
          </Routes>
        </MemoryRouter>
      </ToastProvider>
    </QueryClientProvider>,
...
async function waitForRows(): Promise<void> {
  await waitFor(() => {
    expect(screen.getByText('주문/결제')).not.toBeNull();
  });
}
...
describe('CategoriesPage — 행 액션은 각자의 권한을 탄다 (EXC-03)', () => {
  ...
  it('create 권한이 없으면 등록 CTA 가 사라진다', async () => {
    seedPermissions(['create']);
    renderPage();
    await waitForRows();

    expect(screen.queryByRole('button', { name: '유형 추가' })).toBeNull();
  });
  ...
  it('권한이 다 있으면 추가·수정·삭제가 함께 보인다', async () => {
    ...
    expect(screen.queryByRole('button', { name: '유형 추가' })).not.toBeNull();
    expect(screen.queryAllByRole('button', { name: /수정$/ }).length).toBeGreaterThan(0);
    expect(screen.queryAllByRole('button', { name: /삭제/ }).length).toBeGreaterThan(0);
  });
});
