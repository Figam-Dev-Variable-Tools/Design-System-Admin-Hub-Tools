...
> 소유: A01 (Architecture AI) · 강제: A81(ESLint/Prettier) · 측정: A83(Clean Code Inspector) · 승인: A42(G6)
>
> **A40(중급) · A41(고급) · A30(Storybook) 은 작업 착수 전 이 문서를 읽는다.**
...
| 페이지 | **다른 페이지를 import 하지 않는다** | A83 blocker → PR 차단 |
| 공통 모듈 | **도메인을 모른다** (회원/권한 타입 import 금지) | A83 blocker |
...
이 4종은 조직의 핵심 방어선이다. `// eslint-disable` 로 덮지 않는다 —
...
**도입 결정은 A80(Dependency Manager)의 ADR 이 내린다. A40/A41 이 임의로 추가하지 않는다.**
라이브러리가 필요하면 A80 에 change_request 를 낸다.
...
| 질문 | 주체 | 하지 않는 것 |
|---|---|---|
| 무엇을 만드는가 (구현) | **A40** Frontend Engineer (Mid) | 대규모 리팩터 · 의존성 임의 추가 |
| 무엇을 도입하는가 (정책) | **A80** Dependency Manager | 라이브러리 사용 코드 작성 |
| 어떻게 정리·집행하는가 | **A41** Frontend Engineer (Senior) | 동작 변경 · 신규 기능 |
| 얼마나 나쁜가 (측정) | **A83** Clean Code Inspector | 코드 수정 |
| 무엇을 모듈로 뽑는가 | **A84** Module Extraction Architect | 코드 수정 |
| 통과인가 (G6) | **A42** Code Reviewer | 직접 수정 |
...
- [ ] 필요한 토큰이 `tokens.json` 에 전부 있는가 — 없으면 A20 에 요청 (임의로 하드코딩하지 않는다)
- [ ] 쓰려는 컴포넌트가 이미 `@tds/ui` 나 `shared/ui` 에 있는가 (`pnpm reuse:check`)
- [ ] 라이브러리가 필요하면 A80 의 ADR 이 있는가
