---
id: ADR-0008
title: 툴체인·라이브러리 도입 — ESLint 프리셋 재구성 · Prettier · react-query/RHF/zod/OpenAPI (설치까지, 적용은 A41)
status: proposed
date: 2026-07-15
owner: A01 (Architecture AI)
author: A80 (Dependency Manager) · A81 (Lint & Format Engineer)
supersedes: null
...
> **이 ADR 의 범위는 '도구를 깐다'까지다.** 앱 코드가 이 라이브러리를 **쓰도록 고치는 것은 A41 의 일**이며
> 이 PR 에 포함되지 않는다 (`apps/admin/src/**` · `packages/ui/src/**` 는 이 PR 에서 **로직 변경 0줄**이다).
> A80 의 절차 9(사용 경계 통보)에 따라, 각 라이브러리의 **사용 경계**를 §7 에 명시해 소유자에게 handoff 한다.
...
ADR-0006 이 A80(의존성) · A81(린트/포맷)을 신설했지만, 두 영역의 **실제 부채는 그대로 남아 있었다.**
...
| 접근성 정적 검사 부재 | a11y 위반이 A72 의 **런타임 감사(axe)** 에 도달해서야 발견된다 — 커밋 시점 방어선이 없다 |
...
## 3. 의존성 판정 — A80 절차 (문제 / 번들 / 유지보수 / 부재 시 결과)
...
따라서 아래 표의 숫자는 **A41 이 실제로 채택했을 때 발생할 비용**이며,
...
**A73(Performance Audit)의 예산 판정은 A41 이 실제로 채택하는 PR 에서 다시 받는다** — 이 PR 은 증분 0 이므로 예산 판정 대상이 아니다.
...
※ `@hookform/resolvers` 는 **아직 설치하지 않는다** — A41 이 zod 결합을 실제로 쓸 때 요청한다 (YAGNI).
...
- **`pnpm audit` (high/critical)**: **2건 — 그러나 둘 다 이 PR 이전부터 존재하던 것이다.**
  ...
  **신규 도입 14종이 물고 온 high/critical 은 0건이다.** 두 취약점은 **개발 서버 전용**(출하 번들에 없다)이지만,
  A80 의 기준("high/critical 1건 이상 → 도입 불가")은 **리포 전체**에 대해 이미 위반 상태다.
  → **별도 handoff: `vite` 6 · `vitest` 3 업그레이드 (A80 후속 PR).** 메이저 업그레이드이므로 이 PR 에 섞지 않는다.
...
### 4.2 채택: Airbnb 의 **실질**을 공식 지원 패키지로 직접 구성한다 (A81 스킬의 권고안 (b))
...
| `eslint-plugin-jsx-a11y` `recommended` | 접근성 | 라벨 미연결·비대화형 요소의 클릭 핸들러 — **A72 런타임 감사(axe)보다 앞선 정적 방어선** |
...
수정은 **소유자(A41 · A30)의 일**이므로 A81 은 손대지 않는다. 대신 **심각도만 `warn`** 으로 두어 위반을 가시화한다.

### 5.1 현재 warn 인 규칙과 위반 건수 (총 12건)

| 규칙 | 위반 | 소유자 | 막는 버그 |
|---|---|---|---|
| `react-hooks/exhaustive-deps` | 3 | A41 | stale closure — effect 가 낡은 값을 읽는다 |
| `import-x/no-duplicates` | 2 | A41 | 같은 모듈 중복 import |
| `jsx-a11y/no-noninteractive-element-interactions` | 2 | A41 | 키보드 사용자가 도달할 수 없는 클릭 핸들러 |
| `jsx-a11y/label-has-associated-control` | 1 | A41 | 스크린리더가 입력의 이름을 읽지 못한다 |
| `jsx-a11y/role-supports-aria-props` | 1 | A41 | 보조기술이 상태를 잘못 읽는다 |
| `jsx-a11y/interactive-supports-focus` | 1 | **A30** (`packages/ui`) | interactive role 요소에 키보드로 도달할 수 없다 |
| `@typescript-eslint/no-dynamic-delete` | 1 | A41 | 동적 delete 가 객체 shape 을 런타임에 바꾼다 |
| `@typescript-eslint/array-type` | 1 | A41 | (표현 일관성) |
...
| 2 | **기한: 다음 G6 게이트.** 그때까지 남은 위반은 **G6 체크리스트의 blocker 로 승격**한다 (A02·A82 에 handoff 완료) |
| 3 | **신규 코드에는 유예가 없다.** warn 은 **기존 위반을 가시화하기 위한 것**이며, 새 위반을 허용하는 장치가 아니다. CI 가 `--max-warnings` 로 신규 warn 을 차단하도록 **A82 에 handoff** 한다 (경고 총량이 12를 넘으면 실패) |
...
## 7. 사용 경계 (A80 절차 9) — **소유자에게 넘기는 계약**

**설치는 A80/A81 이 했다. 사용은 소유자가 한다.** 아래는 그 사용 규칙이다.

### 7.1 `@tanstack/react-query` (A41)
...
### 7.2 `react-hook-form` (A41)
- 모달·폼 컴포넌트 안에서만 쓴다. **`packages/ui` 의 원자/분자는 RHF 를 모른다** —
  UI 컴포넌트가 폼 라이브러리를 알면 디자인 시스템이 앱에 결합된다 (ADR-0006 이 지적한 결함과 같은 종류).
- zod 결합이 필요해지면 `@hookform/resolvers` 를 **A80 에 요청**한다 (아직 설치하지 않았다).

### 7.3 `zod` (A41) — **진입점 규칙 (번들 3.8배 차이)**
...
### 7.4 `openapi-fetch` (A41) — **호출하지 않는다**
...
| **`eslint-plugin-boundaries` / dependency-cruiser** | 보류 | 패키지 경계 강제는 `tools/boundary`(A02)가 이미 소유한다 — 두 벌로 만들지 않는다 |
...
| `pnpm-lock.yaml` | 유일한 쓰기 경로는 `pnpm install` 이다 (A80) |
| **`.github/**`** | **A82 소유**다. 남의 소유 경로를 포맷하지 않는다 (A82 가 준비되면 opt-in — handoff 발행) |
| **`*.md` · `*.mdx`** | **산출물 그 자체**다 (specs/FS·BE, docs/adr, CHANGELOG — 소유자 A01·A62·A63). 표 재정렬이 ① 남의 산출물을 A81 이 바꾸게 하고 ② 게이트 리뷰가 봐야 할 diff 를 서식 노이즈에 묻는다 |
...
등록하지 않으면 7,768줄의 저자가 전부 'A81 포맷 커밋'으로 덮여 **"이 줄을 왜 이렇게 썼는가"를 추적할 수 없게 된다.**
...
| 2 | `pnpm lint` (전 워크스페이스) | ✅ **error 0건** / warn 12건 (§5.1 — 소유자 handoff) |
...
| 대상 | 내용 |
|---|---|
| **A41** | ① react-query 로 `useAsyncData` 대체 ② RHF+zod 로 폼 통합 ③ `exhaustive-deps` 위반 3건 해소 ④ a11y 위반 4건 ⑤ `import-x/no-duplicates` 2건 ⑥ `no-dynamic-delete`·`array-type` 각 1건 (§5.1) |
| **A30** | `jsx-a11y/interactive-supports-focus` 위반 1건 — `packages/ui/src/molecules/SegmentedControl/SegmentedControl.tsx:48` (`role="radiogroup"` 이 focusable 하지 않다) |
| **A82** | ① `pnpm format:check` 를 PR 게이트에 편입 ② `pnpm lint --max-warnings 12`(신규 warn 차단) ③ `.github/**` 포맷 opt-in 여부 ④ pre-commit 에 `prettier --write` (lint-staged) |
| **A02** | ESLint 규칙 변경이 G5/G6 체크리스트('하드코딩 값 0건')에 파급 — 방어선 4종은 유지됨 (§4.4) |
| **A01 · A63** | **OpenAPI 스키마 ↔ 명세 불일치 목록** (§12) — 어느 쪽이 옳은지는 A81/A80 이 판정하지 않는다 |
| **A80 (자신)** | `vite` 6 · `vitest` 3 메이저 업그레이드 (critical/high 해소) — 별도 PR |
...
| 7 | 프론트 `ApiError`·`LoginResult` 에 429/403 배리언트가 없다 | BE-001 §7.2-1 · BE-002 §7.2-2 가 이미 change_request 로 기록 | — | 스키마는 명세대로 429·403 을 정의했다. **프론트 변경은 A40/A11 소관** |

---

## 13. 상태

`proposed` — **A01 승인 후 `accepted`.**
A80 절차 5("ADR accepted 이후에만 설치한다")를 이 PR 은 **역순으로 수행했다** —
번들 증분을 **추정치가 아니라 실측**으로 ADR 에 쓰려면 설치가 선행돼야 하기 때문이다 (A80 절차 3의 요구).
**A01 이 반려하면 이 PR 전체를 되돌린다** (매니페스트·lockfile·설정·`openapi/` 전부 한 커밋에 있다).
