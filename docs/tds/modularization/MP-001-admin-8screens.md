...
> **소유**: A84 (Module Extraction Architect) · **게이트**: G0 · **검수**: A00
> **대상 화면(오너 확정)**: 로그인 · 대시보드 · 회원 목록 · 회원 상세 · 관리자 관리 · 권한 관리 · 고객 설정 · 로그인 이력
> **입력**: `reports/reuse/module-candidates-2026-07-15.json` (A75 scan) · `contracts/*.contract.json` 15종 · `packages/ui/src/**` · `reports/code-quality/2026-07-14.json`
> **상태**: 계획 확정 — **집행은 A18/A30/A41**. A84는 코드를 고치지 않는다 (SKILL 하드 바운더리).
...
`page-module-pipeline.md` ③ 은 *"G5 통과 후 페이지의 로컬 구현은 모듈 소비로 교체한다 (사본 잔존 시 A75 중복률 SLO ≤ 3% 위반)"* 라고 규정한다.
...
A75 `reuse:check` 기계 판정 첨부 (유사도 < 60% → CREATE_OK, 즉 기존 15종의 중복이 아님):
...
**`TriStateCheckbox` 는 이미 `shared/ui` 에 있다.** 그런데 `MembersTable.tsx:67-99` 와 `AdminsTable.tsx:50-82` 가 **같은 위젯을 손으로 다시 짰다** (`SelectAllCheckbox`). 이것이 A83 축3 이 잡은 30줄 클론(`clone:f8657271fc8e9492`)의 정체다.
...
**A84 는 이 이유로 어떤 코드도 고치지 않았다.** 반쪽 재활용은 오너 지시 위반이다.

---

## 4. 실측 부채 (A83 재측정 — 2026-07-15)

`pnpm quality:check` → **blocker 0 · major 10** (중복 7 · 복잡도 3). *A40 의 `filterHeadingStyle` 4벌 승격은 완료 확인됨 — 로컬 사본 0건.*

| 축 | 건수 | 최대 | 소유자 |
|---|---|---|---|
| 축3 중복 (30줄·2회+) | **7** | 11곳 반복 1건 | A40/A41 ×6 · **A30 ×1** |
| 축4 복잡도 (>15) | **3** | **24** | A40/A41 ×3 |
| 축5 죽은 코드 | 0 | — | (유지) |
...
| `Button.stories.tsx:119` **×11곳** | 스토리 보일러플레이트 11벌 | **A30 소유.** `play` 헬퍼 추출 |
...
**10건 전부 A40/A41·A30 소유다. A84 소유 파일에는 위반이 0건이다** (`boundary:check` 실측). 그래서 A84 는 상환하지 못한다 — CR 로 넘긴다.
...
```
[1] A18  계약 EXTEND 7건            ← CR-2026-0715-004
...
6. 승격 후 **로컬 사본 삭제까지가 1건이다.** 사본이 남으면 A75 중복률 SLO(≤3%) 위반이고 축5(죽은 코드)가 0에서 올라간다.

---

## 7. 3자 대조 결과 (SKILL 절차 3)
...
**A84 는 소스를 한 줄도 바꾸지 않았다.** 위 수치는 집행 전후 비교의 기준선이다.
