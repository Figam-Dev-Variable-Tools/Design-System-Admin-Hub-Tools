...
- 소유: A50 (Figma Plugin Engineer, `tools/figma-plugin/**` + 이 문서)
- 검수: A56 (Figma Reviewer, G7) — Detached 스캔·바인딩률 판정의 기준 문서
- 소비: A51/A52/A53 (생성 결과 위에 Variant 내용·레이아웃·화면을 채움), A71 (드리프트 감시)
...
병기해 드리프트 검수(A71)가 데이터로 대조할 수 있게 한다:
...
3. **Variant 매트릭스 자리** — CONTENT_W × GRID×30 프레임, fill = `color/surface/raised`,
   ...
   `Variant 매트릭스 — '계약 → Variant Property 동기화' 실행 후 A51이 채움` (14 muted).
   생성기는 이 자리 프레임만 만들고 Component Set을 여기로 옮기지 않는다 (A51 소유).
...
- 예외 2건(5.2의 line-height/font-family)과 헤어라인 외의 Detached는 G7 검수(A56)의
