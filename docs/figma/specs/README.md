...
**Figma 파일 에이전트(A51~A55)는 자기 영역의 상태를 이 디렉터리에 미러로 기록할 의무**를
진다 (ADR-0001 결정 5). A56(Figma Reviewer)의 G7 검수와 A70(Visual Regression)의 VRT는
Figma 파일이 아니라 **이 미러와 exports/ 기준 PNG**를 evidence로 판정한다.

```
docs/figma/specs/
├── components/   A51 — Component / Variant / Component Set 미러
├── variables/    A52 — Color/Type/Radius/Spacing/Shadow Variables 미러
├── layout/       A53 — Auto Layout · Constraints · Grid · Responsive 미러
├── prototype/    A54 — Prototype · Interaction · Animation 미러
├── icons/        A55 — Icon Component + Variable 연결 미러
└── exports/      VRT 기준 PNG (A70이 pnpm vrt로 Storybook 스크린샷과 픽셀 비교)
```

각 하위 디렉터리의 쓰기 소유자는 위 한 명뿐이다 (agents.json owns 참조).
`exports/`에는 A51~A55가 자기 영역의 기준 이미지를 내보낸다.
...
4. Variables(A52)는 tokens.json에서 플러그인으로 생성한 결과만 미러링한다 —
   Figma에서 수동 생성한 Variable은 그 자체가 드리프트다 (A71 감시 대상).

## 4. 소비자

| 소비자 | 용도 |
|---|---|
| A56 (Figma Reviewer) | G7 체크리스트 판정 evidence (docs/_templates/checklists/G7.md) |
| A70 (Visual Regression) | `pnpm vrt` 기준 이미지 → `reports/vrt/` |
| A74 (Contract Test) | Contract ↔ Figma 축 대조 (Property 이름/타입/값) |
| A71 (Design Drift) | Storybook ≠ Figma ≠ Token 드리프트 상시 감시 |
