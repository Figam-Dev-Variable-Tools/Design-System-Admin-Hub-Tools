...
- 입력: 계약 31종 · FS 14건 · 테스트 파일 95개 · 스토리 파일 41개
- **단언을 가진 실행 단위(= 테스트): 982건** / 단언 없는 실행 단위: 2건
...
| 2 | 계약 states 커버리지 (contracts/*.contract.json → states[]) | **blocker** | 97 | 97 | 0 | 미커버 상태 0건 (전수) | G5·G6 | PASS |
| 3 | 계약 events.blockedWhen 커버리지 (금지 동작의 비발생 단언) | **blocker** | 16 | 16 | 0 | 미커버 차단 조건 0건 (전수) | G5·G6 | PASS |
...
|---|---|---|---|---|
| @tds/admin | `apps/admin` | **581** | 0 | PASS |
| @tds/ui | `packages/ui` | **338** | 2 | PASS |
