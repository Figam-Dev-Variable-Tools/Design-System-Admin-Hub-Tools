...
- 입력: 계약 35종 · FS 14건 · 테스트 파일 96개 · 스토리 파일 45개
- **단언을 가진 실행 단위(= 테스트): 1004건** / 단언 없는 실행 단위: 2건
...
| 2 | 계약 states 커버리지 (contracts/*.contract.json → states[]) | **blocker** | 107 | 107 | 0 | 미커버 상태 0건 (전수) | G5·G6 | PASS |
...
|---|---|---|---|---|
| @tds/admin | `apps/admin` | **569** | 0 | PASS |
| @tds/ui | `packages/ui` | **372** | 2 | PASS |
