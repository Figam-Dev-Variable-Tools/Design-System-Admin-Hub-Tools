...
- 입력: 계약 21종 · FS 14건 · 테스트 파일 84개 · 스토리 파일 31개
- **단언을 가진 실행 단위(= 테스트): 874건** / 단언 없는 실행 단위: 2건
...
| 2 | 계약 states 커버리지 (contracts/*.contract.json → states[]) | **blocker** | 67 | 67 | 0 | 미커버 상태 0건 (전수) | G5·G6 | PASS |
| 3 | 계약 events.blockedWhen 커버리지 (금지 동작의 비발생 단언) | **blocker** | 13 | 13 | 0 | 미커버 차단 조건 0건 (전수) | G5·G6 | PASS |
...
|---|---|---|---|---|
| @tds/admin | `apps/admin` | **583** | 0 | PASS |
| @tds/ui | `packages/ui` | **228** | 2 | PASS |
