...
- 입력: 계약 15종 · FS 4건 · 테스트 파일 30개 · 스토리 파일 25개
- **단언을 가진 실행 단위(= 테스트): 371건** / 단언 없는 실행 단위: 0건
...
| 4 | FS 예외 7축 커버리지 (요소 × 축 격자 — 동작이 정의된 칸만 · 래칫) | major | 137 | 759 | 622 | 미커버 칸 0건 (major) · **커버 칸 수 후퇴 = blocker** | G6 | VIOLATED |
...
|---|---|---|---|---|
| @tds/admin | `apps/admin` | **134** | 0 | PASS |
| @tds/ui | `packages/ui` | **174** | 0 | PASS |
...
| … 외 **610건** | 전수 목록은 JSON 리포트 `gaps[]` 참조 | |
