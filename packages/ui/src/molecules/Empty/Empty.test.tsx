...
describe('Empty — 계약 states[]', () => {
  it('Empty: default 상태 — role="status" 라이브 영역에 사유 문구를 렌더한다', () => {
    ...
    expect(region.textContent).toContain('등록된 회원이 없어요');
  });
});
...
describe('Empty — 상태 분기 (STATE-05)', () => {
  ...
  it('Empty: 진짜 비어있음 — createVerb 를 문구에 반영한다 ("접수된 문의가 없어요")', () => {
    render(<Empty label="문의" createVerb="접수" />);
    // '문의' 는 받침 없음(ㅢ) → '가'
    expect(screen.getByRole('status').textContent).toContain('접수된 문의가 없어요');
  });
  ...
  it('Empty: 검색 결과 없음 — hasQuery 가 filter/empty 보다 우선하고 조건 문구를 보인다', () => {
    ...
    expect(screen.getByRole('status').textContent).toContain('조건에 맞는 회원이 없어요');
  ...
  it('Empty: 필터 결과 없음 — hasActiveFilters 만 true 면 필터 문구를 보인다', () => {
    ...
    expect(screen.getByRole('status').textContent).toContain('필터에 맞는 공지가 없어요');
  });
});
