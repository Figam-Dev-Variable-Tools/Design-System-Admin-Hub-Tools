# pages — Storybook Pages 탭 (A32 소유, 게이트 G5)

**조립 전용** 영역이다. `../src`의 public 컴포넌트만 조합해 Dashboard / User / Product / Settings 등
페이지 단위 시나리오 스토리(`pages/**/*.stories.tsx`)를 만든다.

- 신규 컴포넌트 생성 금지 — 필요한 컴포넌트가 없으면 작업을 멈추고 A18(Contract Engineer)에게
  change_request(orchestration/schemas/change-request.v1.json)를 발행한다.
...
| 조립 원칙 | `../src` public 컴포넌트만 조합. **이 폴더에서 신규 컴포넌트를 만들지 않는다** — 없으면 A18에게 change_request 발행 후 대기 |
...
모듈별 G5 승인 여부는 `orchestration/state/<component>.json`의 `gates.G5.status`로 확인한다.
