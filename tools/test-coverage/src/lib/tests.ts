 ...
 * 그래서 A77 은 러너의 판정이 아니라 **소스에 실제로 존재하는 단언**을 센다.
 * (러너 JSON 이 있으면 교차 검증만 한다 — `ingestRunnerReport`. 판정 권한은 주지 않는다.)
 *
 * 검증 대상(apps/** · packages/** · e2e/**)은 전부 **읽기 전용**이다 — A77은 테스트를 대신 쓰지 않는다.
 */
import {
  ASSERTION_PATTERNS,
...
export interface TestUnit {
  ...
  file: string;
  line: number;
  /** 테스트/스토리 이름 — 대조의 유일한 키다 (A85 명명 규칙의 전제) */
  name: string;
 ...
 *
 * **이 함수는 A77의 거짓 음성(false negative) 버그를 고친 자리다.**
 ...
 *     };
 *
 * 결과: **단언이 실제로 있는데 `assertionFree` 로 집계됐다.** A30은 이 오탐을 피하려고 자기
 * 헬퍼 시그니처를 비구조분해(`(ctx: PlayCtx)`)로 바꾸고 그 사유를 주석으로 남겼다 —
 * **도구를 만족시키려 컨벤션을 비튼 것이다. 꼬리가 개를 흔들었다.**
 *
 * A77은 "단언 없는 실행 단위는 테스트가 아니다"라는 원칙 위에 서 있다. 그 판정이 파서 버그로
