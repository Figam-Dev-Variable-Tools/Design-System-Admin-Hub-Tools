...
> 소유: **A71 Design Drift AI** (`orchestration/registry/agents.json`)
...
| `hardcoded-values` | `packages/ui/src` 하드코딩 스캔 — HEX 색상 · px 수치 · rgb()/hsl() 함수. **contract-test(A74)와 동일 정규식**(`src/rules.ts`) — 단 여기서는 warning으로 수집만, 차단 판정은 A74 관할 | warning |
...
- 정규식 목록(`src/rules.ts`)을 바꾸려면 A01 승인 후 tools/contract-test 쪽과 **동시 수정**해야 한다.
