...
> `712c30b` 가 발급에 멱등키를 붙였다: `ApiKeysPage.tsx:96,122-123,130,156`(시도 단위 `idempotencyKeyRef` — **성공해야 버리고 실패에는 남긴다**) → `queries.ts:30,37-38` → `data-source.ts:107-141`. 아래 '프론트 후속(A11)' 이 요구한 그대로다.
...
> **현재는 브라우저 안 `Map` 이라 §7.1(평문 미보관) 위반이 아직 발생하지 않았다**(서버가 없다). **연동 전에 A63 이 정본을 정해야 한다** — 정하지 않으면 §7.1 을 어기는 서버가 만들어진다. 교차참조: NFR-069 §5 #20 · FS-069 §7 #8.
