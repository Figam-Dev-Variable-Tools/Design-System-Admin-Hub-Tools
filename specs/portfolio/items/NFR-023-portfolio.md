...
| 판정 기준일 | **2026-07-17 · HEAD = `a5c2639`** (PR #22·#24·#26·#28·#30·#32·#34 머지 후). 직전 판정은 `4b805ad` 기준이었다. **이번 기준 갱신으로 P0 2건이 뒤집혔다 — gap 8 → 6**: **A11Y-11**(gap → **pass** — PR #30 이 `ImageUploadField` 의 required 를 **접근가능 이름**으로 잇는다: `requiredNameSuffix` `ImageUploadField.tsx:55` → `aria-label` `:250`. `aria-required` 가 아닌 이유는 드롭존이 `<button>` 이고 그 속성이 `role=button` 미지원이기 때문이다 — `:44-54`) · **MOTION-03**(gap → **종속** — `ToggleSwitch.css:79-84` 에 reduced-motion 게이트가 실재하게 됐다). **STATE-02 · IA-02 · IA-04 · IA-13 · EXC-03 · EXC-04 는 gap 그대로다**(§2 · §5) |
...
> **규약의 예외 하나(명시)**: `상속` 항목이라도 **코드 대조로 위반을 직접 확인했고 그 표면이 이 화면에 실재하면 `gap` 으로 적고 범위를 '앱 전역'으로 표기한다.** `종속` 으로 덮으면 검증된 P0 미충족이 문서에서 사라진다. **`a5c2639` 기준 — 이 예외에 해당하는 행이 0건이 됐다**: 직전까지 A11Y-11 · MOTION-03 두 건이 여기 해당했으나 PR #30·#26 이 **둘 다 실제로 고쳤다**(`ImageUploadField.tsx:55,250` · `ToggleSwitch.css:79-84`).
>
...
>
> ⚠ **대표 이미지(`ImageUploadField`)는 `aria-required` 를 조회해서도 안 된다.** AT 에 보이는 컨트롤이 `<button>` 이고 `aria-required` 는 `role=button` 이 지원하지 않는 속성이라(ARIA 1.2) **영원히 null 이며 그것이 의도된 설계다**(근거 `ImageUploadField.tsx:44-54`). 이 필드의 required 는 **접근가능 이름**으로 전달된다 — `requiredNameSuffix`(`:55`)가 `:250` 의 `aria-label` 에 실린다. 판정은 `getByRole('button', { name: /대표 이미지 \(필수\)/ })` 매치 여부로 한다.
...
| A11Y-11 | A11Y | 직접(+앱 전역) | **6개 중 5개 충족 — 1개 미충족.** ① **describedby 절 — 충족**: `aria-invalid` 를 쓰는 이 화면의 모든 컨트롤이 `aria-describedby` 로 이유를 연결한다 — `PortfolioFormPage.tsx:134-137`(분류) · `:156-157`(일자) · `:172-173`(제목) · `:187-188`(고객사), `TextareaField.tsx:66-67`(소개, 자체 배선), `ImageUploadField.tsx:211`·`ImageGalleryField`(`aria-invalid` 를 쓰지 않고 describedBy 만 — 짝 없는 위반이 아니다). 회귀는 `shared/a11y-guard.test.ts` 가 소스 전수 스캔으로 고정한다. ② **required 절 — F3a 가 대부분 닫았다**: `FormField.tsx:50-56` `withAriaRequired` 가 `required` 를 **런타임 `cloneElement`** 로 자식 컨트롤의 `aria-required` 에 주입한다(대상 판별 `:36-41`). 이 폼의 필수 6개 중 **5개가 주입 대상**이다 — 분류(자식이 DS `SelectField`, `PortfolioFormPage.tsx:130`) · 일자(`:150` `input`) · 제목(`:164` `input`) · 고객사(`:179` `input`) · 소개(`TextareaField.tsx:64-65` 가 native `required`+`aria-required` 를 스스로 건다). ③ **남은 1개였던 대표 이미지 — PR #30 에서 닫혔다.** `ImageUploadField` 가 `FormField` 를 거치지 않는다는 사실은 그대로지만, DS 가 **`aria-required` 가 아니라 접근가능 이름으로** 이 축을 해결했다 — `requiredNameSuffix(required)`(정의 `ImageUploadField.tsx:55`, export `index.ts:2`)가 드롭존 `<button>` 의 `aria-label` 에 실린다(`:250` — `` aria-label={`${label}${requiredNameSuffix(required)} 이미지 업로드 — 클릭하거나 파일을 끌어다 놓으세요`} ``). 이 폼의 소비는 `PortfolioMediaFields.tsx:51-59` `<ImageUploadField label="대표 이미지" required …>` 이므로 이름이 **'대표 이미지 (필수) 이미지 업로드 — …'** 가 된다. **⚠ 이전 판정('required 미주입 잔여 결함')은 낡았을 뿐 아니라 요구하던 해법이 틀렸다** — 주석 `ImageUploadField.tsx:44-54` 가 근거를 못 박는다: AT 에 보이는 컨트롤이 `<button>`(드롭존)인데 **`aria-required` 는 `role=button` 이 지원하지 않는 속성**(ARIA 1.2 — textbox/checkbox/combobox/…)이라 얹으면 거짓 시맨틱이자 axe `aria-allowed-attr` 위반이고, 진짜 `<input type="file">` 은 `aria-hidden`+`tabIndex={-1}` 트리거라 그쪽에 줘도 **아무에게도 닿지 않는다**. 접근가능 이름이 유일하게 정직한 경로다. 시각 `*` 마커는 그대로 두고 이름에만 더했다. (`ImageGalleryField`(본문 이미지)는 `required` 를 넘기지 않으므로(`PortfolioMediaFields.tsx:61-69`) suffix 가 빈 문자열이라 이름이 그대로다 — 무관하다) | ① `aria-describedby` 없는 `aria-invalid` grep = 0 → **통과**(가드 테스트가 강제). ② 빈 폼 제출 후 RTL/AT 로 입력의 `aria-describedby` === `role=alert <p>` id → **통과**. ③ **⚠ grep 으로 판정하지 말 것** — 주입은 런타임이라 소스에 리터럴이 없다. RTL 로 렌더해 `getByLabelText('분류')`·`('일자')`·`('제목')`·`('고객사')`·`('소개')` 의 `aria-required` === `'true'` → **5개 통과**. ④ 대표 이미지는 **`aria-required` 를 조회하지 말 것**(role=button 미지원 속성이라 영원히 null 이고, 그것이 의도다) — `getByRole('button', { name: /대표 이미지 \(필수\) 이미지 업로드/ })` 가 **매치되는지**로 판정한다 → **통과** | **pass** |
...
| MOTION-01 | MOTION | 상속 | Modal 표면이 이 화면에 실재한다(삭제·일괄 삭제·충돌·discard 다이얼로그). enter/exit 은 DS `Modal` organism 이 소유하며 이 화면은 소비자다 — 이 화면은 자체 애니메이션을 두지 않는다 | 삭제 다이얼로그 open/close 시 backdrop fade + dialog scale 이 재생되고 exit 후에만 DOM 에서 제거되는지. **참고 — `a5c2639` 사실 갱신**: 이전 관측('`Modal.css` 에 enter/exit 선언 없음 — DS 미도입으로 보인다')은 **더 이상 사실이 아니다**. PR #26 이 구현했다 — backdrop fade(`Modal.css:20-21,30-33`→keyframes `:126-144`) + dialog scale 0.96→1(`:58-59,35-38`→keyframes `:146-168`, `forwards`) + reduced-motion 게이트 `:173-180`. `AnimatePresence` 소비 0건은 여전하나 **모션 부재를 뜻하지 않는다** — CSS-only 이고 'exit 후에만 DOM 제거' 는 네이티브 `onAnimationEnd`(`Modal.tsx:216-218`)가 보장한다. 판정 정본은 DS 문서 | 종속 |
| MOTION-02 | MOTION | 상속 | Toast 표면이 이 화면에 실재한다(§A11Y-01 의 6갈래). exit 애니메이션은 `ToastProvider`/`Toast` 가 소유한다 | toast auto/manual dismiss 시 exit 애니메이션 재생 여부. **참고 — `a5c2639` 사실 갱신**: PR #26 이 **완전 구현**했다 — `.tds-toast--exiting`(`Toast.css:32-37`) + keyframes `:121-131`(opacity 1→0 · `translateY(0)→translateY(var(--tds-space-3))`) + reduced-motion 게이트 `:136-141`. exit duration `{motion.duration.fast}`(150ms) · easing `{motion.easing.accelerate}`. 판정 정본은 DS 문서 | 종속 |
| MOTION-03 | MOTION | 상속 | **PR #26 에서 뒤집혔다 — 게이트가 실재한다.** 이 화면은 `ToggleSwitch` 를 두 곳에서 쓴다 — 목록 노출 토글(`_shared/publishColumn.tsx:20`)과 폼 노출 토글(`_shared/PortfolioMediaFields.tsx:73`). 즉 quality-bar 가 appliesTo 로 지목한 바로 그 표면이 이 화면에 실재한다. **이전 판정의 근거('`ToggleSwitch.css` 파일 전체에 `prefers-reduced-motion` 0건')는 더 이상 사실이 아니다** — `ToggleSwitch.css:79-84` 가 `@media (prefers-reduced-motion: reduce) { .tds-toggle__track, .tds-toggle__knob { transition: none; } }` 로 `:32` background-color·`:56` transform 두 선언을 **둘 다 끈다**. 논거 `:76-78`: 손잡이 transform 은 **움직임**이라 vestibular 장애에 직접 영향하고, 상태(on/off)는 색·위치·`aria-checked` 로 이미 전달되므로 전환 제거 시 정보 손실이 0 이다 — 즉시 최종 위치로 스냅한다. Modal·Toast 도 각자 게이트를 갖는다(`Modal.css:173-180` · `Toast.css:136-141`). 이 화면은 자체 transition/animation 을 선언하지 않는다 | `prefers-reduced-motion: reduce` 를 켠 채(Storybook/Playwright `emulateMedia`) 목록의 노출 토글을 누른다 → **knob 이 미끄러지지 않고 즉시 최종 위치로 스냅한다**. `grep -c 'prefers-reduced-motion' packages/ui/src/atoms/ToggleSwitch/ToggleSwitch.css` = **1**(0 이 아니다). 판정 정본은 DS 문서 | 종속 |
...
| IA-13 | IA | 직접 | **미충족.** 이 화면의 list query state(분류 필터)가 **컴포넌트 `useState` 에만 있다** — `PortfolioListPage.tsx:48` `const [filter, setFilter] = useState(PORTFOLIO_FILTER_ALL)`. 파일에 `useSearchParams`·`useListState` import 0건. **F3b 가 `shared/crud/useListState`(URL 단일 원천)를 34개 화면에 롤아웃했고 같은 섹션의 `support/{downloads,replies,tickets}` 도 소비처가 됐지만**(`DownloadListPage.tsx:87` · `RepliesPage.tsx:76` · `TicketListPage.tsx:129`) **`pages/portfolio/**` 3화면은 전부 그 밖에 남았다.** 정렬은 서버 고정이라 URL 대상이 아니고, page·keyword 는 표면 자체가 없다 | 분류를 '오피스'로 바꾼 뒤 ① F5 → **'전체 분류'로 되돌아간다** ② 행을 눌러 수정 폼에 갔다가 브라우저 Back → **필터가 풀린 목록에 착지한다** ③ 주소창 URL 을 복사해 새 탭에 붙여넣기 → **필터가 재현되지 않는다**(URL 에 `?` 가 아예 붙지 않는다). `grep -rn 'useSearchParams\|useListState' apps/admin/src/pages/portfolio/` = **0**(앱 전체로는 34개 화면 — `a5c2639` 재확인). 대조: 같은 조작을 `/support/downloads` 에서 하면 URL 에 `?category=…` 가 붙고 Back·F5 를 견딘다 | **gap** |
...
| 판정 | 건수 | 요구 ID |
|---|---|---|
| `pass` | **12** | STATE-01 · STATE-04 · TOKEN-01 · **A11Y-11** · FEEDBACK-02 · FEEDBACK-04 · IA-01 · IA-05 · EXC-01 · EXC-02 · EXC-08 · EXC-09 |
| `종속` | **9** | TOKEN-02 · TOKEN-03 · TOKEN-04 · TOKEN-05 · A11Y-01 · A11Y-02 · MOTION-01 · MOTION-02 · **MOTION-03** |
| `n-a` | **3** | COMP-10(텍스트 검색 표면 없음) · FEEDBACK-06(편집 폼 모달 없음) · A11Y-12(좌측 필터 표면 없음) |
| `gap` | **6** | STATE-02 · IA-02 · IA-04 · IA-13 · EXC-03 · EXC-04 |

**검산: 12 + 9 + 3 + 6 = 30** ✓ (STATE 3 · TOKEN 5 · COMP 1 · FEEDBACK 3 · A11Y 4 · MOTION 3 · IA 5 · EXC 6 = 30)

> **`a5c2639` 기준 재판정 결과: P0 2건이 뒤집혔다 — gap 8 → 6.** 둘 다 **DS 가 실제로 고쳤다**:
> - **A11Y-11 gap → pass** — 남아 있던 단 하나(`ImageUploadField` 의 required 가 AT 에 닿지 않음)를 PR #30 이 닫았다. **다만 이전 판정이 요구하던 해법(`aria-required` 주입)이 아니라 접근가능 이름으로 풀었고 그것이 옳다** — AT 에 보이는 컨트롤이 `<button>`(드롭존)인데 `aria-required` 는 `role=button` 미지원 속성(ARIA 1.2)이라 얹으면 거짓 시맨틱 + axe `aria-allowed-attr` 위반이다(근거 `ImageUploadField.tsx:44-54`). `requiredNameSuffix`(`:55`)가 `:250` 의 `aria-label` 에 실려 이름이 '대표 이미지 **(필수)** 이미지 업로드 — …' 가 된다.
> - **MOTION-03 gap → 종속** — quality-bar 가 appliesTo 로 지목한 `ToggleSwitch` reduced-motion 부재가 **해소됐다**(`ToggleSwitch.css:79-84`). 이전 판정의 근거였던 '파일 전체에 해당 미디어 쿼리 0건' 은 이제 거짓이다. ⚠ quality-bar 의 MOTION-03 요구문은 **요구 정본이라 그대로 둔다** — 이제 충족됐을 뿐이다.
>
> **직전 `4b805ad` 기준 기록(보존)**: 뒤집힌 P0 0건. A11Y-11 은 6개 중 5개가 주입돼 남은 것이 `ImageUploadField` 하나로, EXC-04 는 유령 저장·404 가 닫히고 남은 것이 동시성 토큰 하나로 **범위만 좁았다**. 그때 '좁혀진 gap 도 gap' 으로 유지한 판단은 옳았고, **이번에는 그 마지막 하나가 실제로 닫혀서** pass 가 됐다. **P1 3건은 그때 pass 가 됐다**(ERP-13 · EXC-12 · EXC-08 의 멱등키 절 — §3 참조).
>
> **P0 gap 6건 = quality-bar '배치 실패' 사유.** 6건 중 **이 화면 고유는 4건**(STATE-02 · IA-13 · EXC-03 · EXC-04): EXC-03 은 이제 공용 훅(`useRouteWritePermissions`)과 **7곳의 선례**가 있어 이 화면이 소비하기만 하면 되고, IA-13 도 `useListState` 를 **34개** 화면이 이미 쓴다 — **둘 다 '앱 전역 미비'가 아니라 '이 화면의 미소비'로 사유가 바뀌었다.** 나머지 **2건은 앱 전역 범위**(IA-02 · IA-04) — §5 의 '범위' 열이 이를 가른다. **DS 범위였던 2건(A11Y-11 · MOTION-03)은 DS 가 고쳐서 사라졌다.**
...
- [x] **판정 기준일을 `2026-07-17 · HEAD = a5c2639`(PR #22·#24·#26·#28·#30·#32·#34) 로 갱신하고 P0 30건을 전수 재확인했다.** 뒤집힌 P0 **2건 — gap 8 → 6**: **A11Y-11** gap→**pass**(`ImageUploadField.tsx:55,250` — 마지막 하나가 닫혔다) · **MOTION-03** gap→**종속**(`ToggleSwitch.css:79-84` — 게이트 실재). §2.1·§5 #7·#8 을 함께 갱신했다
- [x] **직전 기준에서 '좁혀진 gap 도 gap' 으로 버틴 판단이 옳았음을 확인했다** — `4b805ad` 때 A11Y-11 을 6→1 로 좁히고도 pass 로 올리지 않았고, **이번에 그 마지막 하나가 실제로 닫혀서** pass 가 됐다. 좁혀졌다는 이유로 미리 올렸다면 한 배치 동안 거짓 pass 를 출하했을 것이다
- [x] **낡은 주장 2종을 코드로 반증해 지웠다**: ① 「`ToggleSwitch.css` 파일 전체에 `prefers-reduced-motion` 0건」 → **틀렸다**(`:79-84`, grep = 1) ② 「`Modal.css` 에 enter/exit 선언 없음 — DS 미도입으로 보인다」 → **틀렸다**(`Modal.css:126-168`). 모션은 실재하되 **CSS-only** 다(Motion/framer-motion 은 여전히 미도입). **grep 결과를 그대로 옮겨 적지 않고 파일을 열어 확인했다**
- [x] **A11Y-11 이 '내가 요구하던 방식'으로 고쳐지지 않았다는 이유로 gap 을 유지하지 않았다** — DS 는 `aria-required` 대신 **접근가능 이름**을 골랐고, `aria-required` 가 `role=button` 미지원 속성이라는 점에서 **그쪽이 정확하다**(ARIA 1.2 · axe `aria-allowed-attr`). 요구는 'required 가 AT 에 닿을 것' 이지 '특정 속성을 쓸 것' 이 아니다
- [x] **직전 `4b805ad` 기준 기록(보존)**: P0 판정이 뒤집힌 것은 **0건**(11/8/3/8). **범위가 좁아진 P0 2건**(A11Y-11 6→1 · EXC-04 3사유→1사유)은 gap 을 유지했다. **P1 3건은 pass 로 전환**(ERP-13 · EXC-12 · EXC-08 멱등키 절)했고 §3·§5 를 함께 갱신했다
- [x] **A11Y-11 을 grep 이 아니라 호출부 자식 타입으로 판정했다** — `withAriaRequired` 는 런타임 `cloneElement` 라 소스 grep(앱 전역 1건)은 지표가 아님을 §1.1·§6.4 에 명시했다
- [x] §1.1 표기 규약의 예외(상속 항목의 검증된 위반을 gap 으로 적는 규칙)를 명시했다. **`a5c2639` 기준 해당 건수는 0 이다** — 그 예외로 붙잡아 뒀던 2건(A11Y-11 · MOTION-03)을 DS 가 실제로 고쳤다. 예외 규칙 자체는 유지한다(다음에 또 필요하다)
