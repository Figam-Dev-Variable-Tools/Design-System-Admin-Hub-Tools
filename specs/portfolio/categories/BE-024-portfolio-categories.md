...
| 2 | 모달에 동기 제출 락 없음 + `mutate` 가 `idempotencyKey` 를 비운 채 보낸다(`PortfolioCategoryFormModal.tsx:85-94`). `useCrudForm` 패턴 이식 또는 `useCrudCreate` 에 락 승격(EXC-08 P0 — §7.8) | A11 |
| 3 | ~~`createStoreAdapter` 존재 검사 부재~~ **— 해소됨(F3b)**: `crud.ts:171` `exists()` → `update`/`remove` 409(`:219-221`·`:232-234`) · `fetchOne` 404(`:192-194`). **남은 것은 ① 동시성 토큰**(`updatedAt`/`version` 부재 → 동시 편집 last-write-wins) **② 모달이 409 를 generic 배너로 뭉갠다**(충돌 다이얼로그 미소비 — EXC-04 P0 잔여) | A11 · A63 |
| 4 | `useRouteWritePermissions()` 를 **7곳이 소비하는데 이 화면은 그 밖이다**(선례: `products/categories/ProductCategoriesPage.tsx:181`) — 쓰기 버튼이 권한과 무관하게 렌더(EXC-03 P0 · **이 화면**) | A11 |
| 5 | **계약(§7.3 = 404 `CATEGORY_NOT_FOUND`) ↔ 픽스처 어댑터(409) 불일치** — 대상 부재 시 어느 status 가 정본인지 확정해야 한다. 409 는 `useCrudForm` 의 충돌 다이얼로그를 열고 404 는 generic 배너로 가므로 **소비 화면이 `useCrudForm` 으로 옮겨가는 순간 관측 차이가 드러난다**(지금은 이 모달이 저수준 훅을 써서 우연히 같은 배너로 수렴한다) | A63 · A11 |
