...
| 판정 기준일 | **2026-07-17 · HEAD = `a5c2639`** (PR #22·#24·#26·#28·#30·#32·#34 머지 후). 직전 판정은 `4b805ad` 기준이었다. **이번 기준 갱신으로 P0 1건이 뒤집혔고 방향이 나쁘다 — `FEEDBACK-06` pass → gap** (gap 3 → 4). 이 화면 코드는 그대로이나 **PR #26 이 DS `Modal` 에 넣은 exit 애니메이션이 일방향 latch 를 함께 들여왔다** — `closingRef`(`Modal.tsx:119,124`)에 리셋이 없어 `useModalDirtyGuard` 가 닫기를 veto 하면 모달이 `opacity:0`·`pointer-events:none` 로 영구 고착된다. quality-bar 가 FEEDBACK-06 의 appliesTo 로 `PortfolioCategoryFormModal` 을 직접 지목했으므로 **이 화면이 그 결함의 정면**이다(§2 · §5 #3). MOTION-01/02/03 은 `종속` 유지이나 근거의 '모션 미도입' 관측을 **구현 사실로 교체**했다 |
...
| FEEDBACK-06 | FEEDBACK | 직접 | **`a5c2639` 에서 뒤집혔다 — 배선은 옳으나 DS Modal 의 신규 결함이 이 표면을 망가뜨린다.** ⓐ **배선 자체는 그대로 옳다**: 모달 폼이 `useModalDirtyGuard(isDirty && !saving, onClose)`(`PortfolioCategoryFormModal.tsx:65`)를 쓰고 **`requestClose` 를 `Modal.onClose`(`:104`)와 취소 버튼(`:113`) 둘 다에** 넘겨 4경로(딤·Esc·×·취소)를 덮는다. `discardDialog` 는 모달 **밖**에 렌더(`:154`)해 포커스 트랩을 피한다. pristine 모달은 즉시 닫힌다(`useModalDirtyGuard.tsx:54-56`). ⓑ **그러나 PR #26 의 exit 애니메이션이 일방향 latch 를 들여왔다**: `Modal.tsx:119` `closingRef` 는 `:124` 에서 `true` 로만 세팅되고 **리셋이 어디에도 없다**(`setClosing` 은 `:125` 의 `true` 호출 1곳뿐, 리셋 effect 전무). dirty 상태에서 Esc → Modal 이 latch → exit 애니메이션 → `onAnimationEnd`(`:216-218`) → `onClose()` = 가드의 `requestClose` → dirty 라 `setAsking(true)` 만 하고 **`onClose` 를 부르지 않아 부모가 언마운트하지 않는다** → 사용자가 '머무르기'(`onCancel` → `setAsking(false)`)를 고르면 **Modal 은 마운트된 채 `closing` 이 영구히 true**. `Modal.css:26-28` 이 `pointer-events:none`, `:35-38` dialog exit 이 `forwards` 라 `opacity:0` 고정 → **다이얼로그가 보이지도 않고 닫히지도 않으며 입력이 갇힌다.** 이후 모든 Esc/딤/× 는 `Modal.tsx:123` 에서 즉시 return. reduced-motion/jsdom 변종도 동일 latch(`:129-132` 가 `willAnimate()` false 에 `onClose()` 동기 발사 → 같은 veto) — 이때는 dialog 가 **보이는데 완전 무반응**이다. 근인은 `Modal.tsx:19-25` 가 'onClose() → 부모가 언마운트' 를 설계 전제로 문서화했는데 dirty 가드가 그 전제를 깨고, 깨졌을 때 latch 를 되돌릴 코드가 없다는 것이다(`onClose` 가 `void` 반환이라 Modal 에 veto 신호가 없다). **폭발 반경 9곳** — `onClose={requestClose}` grep: `CreateGroupModal.tsx:154` · `LogoFormModal.tsx:126` · `PasswordChangeModal.tsx:103` · `RoleFormModal.tsx:68` · **`PortfolioCategoryFormModal.tsx:104`** · `ProductCategoryFormModal.tsx:104` · `CreateApiKeyModal.tsx:182` · `RevealKeyModal.tsx:126` · `CategoryFormModal.tsx:106`. quality-bar 가 `PortfolioCategoryFormModal` 을 appliesTo 로 명시한 **바로 그 표면이 이 결함의 사정권**이다 | 모달에 이름을 입력(dirty) → Esc → discard 확인 등장 ✓ → **'취소'(머무르기) 클릭** → **모달이 사라진 채 돌아오지 않는다**(`opacity:0` · `pointer-events:none`). Esc·딤·× 재시도 전부 무반응. 입력한 이름을 되찾을 수 없고 페이지를 떠나는 것 외 복구 경로가 없다. reduced-motion 을 켜면 다이얼로그는 **보이지만** 같은 무반응. **pristine 모달은 정상**(가드가 즉시 `onClose` → 언마운트 → latch 가 살아남을 수 없다) — 그래서 4경로 pristine 재현은 여전히 통과하고 **dirty 경로만 깨진다**. `Toast` 는 같은 `exitingRef` 패턴이나 **버그가 아니다** — veto 경로가 없어 항상 언마운트된다 | **gap** (범위: DS `Modal` — §5 #3) |
...
| MOTION-01 | MOTION | 상속 | 이 화면의 Modal 표면: 폼 `Modal`(`PortfolioCategoryFormModal.tsx:102`) · 삭제 `ConfirmDialog`(`:265`) · discard `ConfirmDialog`(`useModalDirtyGuard.tsx:67`). enter/exit 은 DS `Modal` organism 이 소유한다 — **`a5c2639` 사실 갱신: 이전 관측('`Modal.css` 에 enter/exit 애니메이션이 없고 `AnimatePresence` 도 없다')은 절반이 낡았다.** PR #26 이 모션을 **구현했다** — backdrop fade(`Modal.css:20-21,30-33`→keyframes `:126-144`) + dialog scale 0.96→1(`:58-59,35-38`→keyframes `:146-168`, `forwards`) + reduced-motion 게이트 `:173-180`. `AnimatePresence` 가 없다는 관측만 여전히 참이나 **모션 부재를 뜻하지 않는다** — CSS-only 이고 'exit 완료 후 unmount' 는 네이티브 `onAnimationEnd`(`Modal.tsx:216-218`)가 보장한다. 이 화면은 소비자일 뿐이라 판정은 DS 문서에 종속된다. ⚠ **다만 그 exit 구현이 이 화면의 FEEDBACK-06 을 깨뜨렸다**(latch — §2 FEEDBACK-06 · §5 #3). 모션 요구 자체와는 별개 축이다 | DS Modal 소유 문서의 판정을 따른다 | 종속 |
| MOTION-02 | MOTION | 상속 | 이 화면이 띄우는 토스트(`:192`, `:204`)의 exit 애니메이션은 `ToastProvider`/`Toast` 가 소유한다. **`a5c2639` 사실 갱신**: PR #26 이 **완전 구현**했다 — `.tds-toast--exiting`(`Toast.css:32-37`) + keyframes `:121-131`(opacity 1→0 · `translateY(0)→translateY(var(--tds-space-3))`) + reduced-motion 게이트 `:136-141`. exit duration `{motion.duration.fast}`(150ms) · easing `{motion.easing.accelerate}` | DS Toast 소유 문서의 판정을 따른다 | 종속 |
| MOTION-03 | MOTION | 상속 | 이 화면의 motion 표면: Modal/ConfirmDialog/Toast transition + 버튼 hover transition. 전부 DS 소유이며 reduced-motion 게이트도 DS 가 결정한다. 이 화면은 자체 transition/animation 을 선언하지 않는다(style object 에 `transition` 0건). **ToggleSwitch 는 이 화면에 없다**(포트폴리오 카테고리 모달에 boolean 필드 없음) — MOTION-03 이 명시한 그 위반 표면은 여기 해당하지 않으며, **그 위반 자체도 PR #26 에서 해소됐다**(`ToggleSwitch.css:79-84`). Modal·Toast 도 각자 게이트를 갖는다(`Modal.css:173-180` · `Toast.css:136-141`) | DS 소유 문서의 판정을 따른다 | 종속 |
...
> **`a5c2639` 재판정: P0 gap 이 3 → 4 로 늘었다 — 이 배치에서 유일하게 판정이 나빠진 화면이다.** 뒤집힌 1건: **FEEDBACK-06 pass → gap**. 이 화면의 배선(`useModalDirtyGuard` + `requestClose` 를 `onClose`·취소 양쪽에)은 **바뀌지 않았고 여전히 옳다**. 그런데 **PR #26 이 DS `Modal` 에 exit 애니메이션을 넣으면서 일방향 latch 를 함께 들여왔다** — `closingRef`(`Modal.tsx:119`)가 `:124` 에서 `true` 로만 세팅되고 리셋이 전무하다. dirty 폼에서 Esc → 가드가 veto(`setAsking(true)`) → 부모가 언마운트하지 않음 → 사용자가 '머무르기' → **Modal 이 `closing=true` 로 영구 고착**(`opacity:0` + `pointer-events:none`)돼 **보이지도 닫히지도 않고 입력이 갇힌다**. **quality-bar 가 FEEDBACK-06 의 appliesTo 로 `PortfolioCategoryFormModal` 을 직접 지목했으므로 이 화면이 이 결함의 정면이다**(§2 FEEDBACK-06 행 · §5 #3).
>
> ⚠ **이것은 회귀다** — 직전(`4b805ad`) 기준에서 pass 였고 이 화면 코드는 그대로인데 **DS 변경이 판정을 뒤집었다**. 폭발 반경은 `onClose={requestClose}` 를 쓰는 **9곳 전부**이며 `PortfolioCategoryFormModal.tsx:104` 가 그중 하나다. `Toast` 는 같은 패턴이나 veto 경로가 없어 버그가 아니다.
>
> **직전 `4b805ad` 재판정(보존)**: gap 이 5 → 3 으로 줄었다. 뒤집힌 2건: **STATE-01**(F3b 가 `isFetching` 직결을 끊고 `firstLoading`/`refreshing` 파생을 넣었다 — `PortfolioCategoriesPage.tsx:175-177`) · **A11Y-11**(F3a 의 `withAriaRequired` 런타임 주입 — `FormField.tsx:50-56`, 이 모달의 유일한 required 컨트롤이 네이티브 `<input>` 이라 주입 대상이다). **EXC-04 는 범위가 좁아졌을 뿐**(유령 저장 해소, 동시성 토큰 미해소) **이라 gap 을 유지했다.**
>
> **P0 gap 4건 — quality-bar §How to use 기준 '배치 실패' 사유다.** 성격이 갈린다: **DS 소유 1건**(FEEDBACK-06 — `Modal` 이 latch 를 리셋해야 닫힌다. 이 화면 코드로는 고칠 수 없다)과 **이 화면이 소비하지 않는 것 3건**(EXC-03 은 `useRouteWritePermissions` — 7곳 소비, 최근접 선례 `ProductCategoriesPage` · EXC-08 은 `submitLockRef`+`WriteContext.idempotencyKey` — 정본 `useCrudForm` · EXC-04 는 `useCrudForm` 의 충돌 다이얼로그). **동시성 토큰만이 도메인 타입 변경(BE-024)을 요구하는 진짜 앱 밖 항목이다** — §5 의 '범위' 열 참조.
...
| **모달 dirty 상태에서 브라우저 뒤로가기·새로고침** | 입력 손실 경고 | **미충족 — 어느 요구도 덮지 않는다.** FEEDBACK-06 은 모달 4경로(딤·Esc·×·취소)만, FEEDBACK-04 는 페이지 폼(unload·링크·popstate)만 정의한다. 이 화면은 후자가 없어 모달이 열린 채 뒤로가기하면 **조용히 사라진다** — quality-bar 의 사각지대. §5 #7 |
...
| **3** | **FEEDBACK-06** | **P0** | ⚠ **신규 회귀 (PR #26 · `a5c2639`) — 이 화면 코드는 그대로인데 DS 변경이 pass 를 gap 으로 뒤집었다.** DS `Modal` 의 **일방향 latch**: `closingRef`(`Modal.tsx:119`)가 `:124` 에서 `true` 로만 세팅되고 **리셋이 전무하다**(`setClosing` 은 `:125` 의 `true` 1곳뿐). `Modal.tsx:19-25` 가 'onClose() → 부모가 언마운트' 를 설계 전제로 문서화했는데 `useModalDirtyGuard` 는 dirty 면 `onClose()` 대신 `setAsking(true)` 를 해 **그 전제를 깨고**(veto), 깨졌을 때 latch 를 되돌릴 코드가 없다(`onClose` 가 `void` 반환이라 Modal 에 신호가 없다). **재현**: 이름 입력(dirty) → Esc → discard 확인 → **'취소'(머무르기)** → Modal 이 마운트된 채 `closing=true` 영구 고착 → `Modal.css:26-28` `pointer-events:none` + `:35-38` dialog exit `forwards` 로 **`opacity:0` · 무반응 · 입력 유실**. 이후 Esc/딤/× 는 `Modal.tsx:123` 에서 즉시 return. reduced-motion/jsdom 변종은 `:129-132` 경유로 같은 latch 이며 **보이는데 무반응**이다. **폭발 반경 9곳**(`onClose={requestClose}` grep): `CreateGroupModal.tsx:154` · `LogoFormModal.tsx:126` · `PasswordChangeModal.tsx:103` · `RoleFormModal.tsx:68` · **`PortfolioCategoryFormModal.tsx:104`** · `ProductCategoryFormModal.tsx:104` · `CreateApiKeyModal.tsx:182` · `RevealKeyModal.tsx:126` · `CategoryFormModal.tsx:106`. **`Toast` 는 같은 `exitingRef` 패턴이나 버그가 아니다** — `onDismiss(id)` 를 ToastProvider 큐 `filter` 가 소유하고 veto 경로가 없어 항상 언마운트된다. **즉 vetoing 부모를 가진 쪽만 깨진다.** 최소 수정은 veto 시 latch 리셋이지만 **현재 시그니처로는 Modal 이 veto 를 알 수 없다** — `onClose` 반환값이나 별도 신호가 필요하다(설계 결정 필요 · **미정**) | **DS `Modal`**(이 화면 코드로는 고칠 수 없다). quality-bar 가 FEEDBACK-06 appliesTo 로 `PortfolioCategoryFormModal` 을 직접 지목 | **A40 (DS) · 긴급** |
| 4 | EXC-03 | **P0** | 쓰기 액션 게이팅 부재 — **`useRouteWritePermissions`(`RequirePermission.tsx:45`)는 이제 7곳이 소비하지만**(products 3 · settings 4) 이 화면은 그 밖이다. 읽기 전용 역할이 추가·수정·삭제 버튼을 그대로 보고 누른다. **최근접 선례가 같은 taxonomy+모달 패턴인 `products/categories/ProductCategoriesPage.tsx:181`** 이다. route read 게이팅(①)은 충족 | **이 화면**(공용 훅 + 7곳 선례 존재 — 앱 전역 미비가 아니다) | A11 change_request |
| 5 | EXC-04 | **P0** | **동시성 토큰 부재** — `PortfolioCategory`(`_shared/store.ts:13-16`)에 `updatedAt`/`version` 이 없어 낙관적 토큰을 실을 수 없다(동시 편집 = last-write-wins). 더해 **모달에 409 복구 UI 가 없다** — 저수준 훅을 직접 써 `useCrudForm` 의 충돌 다이얼로그를 물려받지 못했고 `onError`(`PortfolioCategoryFormModal.tsx:80-83`)가 409 를 generic 배너로 뭉갠다. *(유령 저장/삭제는 F3b 의 `createStoreAdapter` 존재 검사(`crud.ts:219-221`·`:232-234`)로 해소)* | 이 화면 + 도메인 모델 | A11 change_request · A63(BE-024 §7.3 서버 404/409 계약 + `version` 필드) |
| 6 | EXC-08 | **P0** | 모달이 `useCrudCreate`/`useCrudUpdate` 를 직접 써 **동기 제출 락이 없고 멱등키를 싣지 않는다**(`PortfolioCategoryFormModal.tsx:85-94` 가 `idempotencyKey` 를 비운 채 mutate). `disabled={saving}` 렌더 가드만 존재 → 빠른 double-Enter 가 비멱등 POST 2건. **F3b 가 자리를 이미 만들어 뒀다** — `WriteContext.idempotencyKey`(`crud.ts:30-42`) + 어댑터 ledger(`:168`·`:201-203`), 정본 패턴은 `useCrudForm.ts:103,201-203`(락)·`:118-123,211`(키) | 이 화면(+ 저수준 훅으로 조립한 다른 모달형 화면) | A11 change_request |
| 7 | (사각지대) | — | 모달 dirty 상태에서 **브라우저 뒤로가기·새로고침** 시 입력이 조용히 사라진다 — FEEDBACK-06(모달 4경로)과 FEEDBACK-04(페이지 폼 3경로) **어느 쪽도 이 경로를 정의하지 않는다.** quality-bar 자체의 공백 | 앱 전역(폼 모달 전부) | A64(quality-bar 개정 검토) · A11 |
| 8 | COMP-01 | P1 | 행 수정·삭제가 raw `<button>` + `buttonStyle('ghost')` 로 조립(`:96`, `:127-148`) — DS `<Button>` 미사용 | 이 화면(+ COMP-01 appliesTo 목록) | A11 |
| 9 | IA-03 | P1 | breadcrumb 부재로 `/portfolio/categories` 와 `/products/categories` 의 AppHeader 제목이 **둘 다 '카테고리'** — 제목만으로 구분 불가 | 앱 전역(AppHeader) | A11 · A40 |
...
| 11 | ERP-06 | P1 | 0-usage 배지 문구가 화면 간 불일치 — 이 화면 '미사용'(`types.ts:15`) vs 문의 유형 '사용 안 함'(`support/_shared/domain.ts:43`) | 두 화면 | A11 |
| 12 | EXC-06 · EXC-07 · EXC-20 | P1 | 실패가 status 로 갈리지 않는다 — 403/404/409/422/429/500 이 전부 같은 문구. 422 field error 매핑 없음. 5xx 참조 코드 없음 | 앱 전역(+ 이 화면 3개 실패 표면) | A11 · A63 |
| 13 | EXC-05 · EXC-11 · IA-14 | P1 | 클라이언트 타임아웃 상한(`AbortSignal.timeout` 0건)·오프라인 감지(`navigator.onLine` 0건)·반응형 선언 부재 | **앱 전역** | A40 · A11 |
| 14 | COMP-12 | P2 | 이름 입력에 글자 수 카운터 없음 — `maxLength=40`(`:136`)이 예고 없이 입력을 멈춘다. `FormField.counter` prop 미사용 | 이 화면 | A11 |
...
- [x] **판정 기준일을 `2026-07-17 · HEAD = a5c2639`(PR #22·#24·#26·#28·#30·#32·#34) 로 갱신하고 P0 30건을 전수 재확인했다.** 뒤집힌 P0 **1건 — `FEEDBACK-06` pass → gap**(gap 3 → 4). §2.1·§5 #3·§4.2 를 함께 갱신했다
- [x] **⚠ 판정이 나빠진 것을 숨기지 않았다.** 이 화면 코드는 한 줄도 바뀌지 않았고 배선(`useModalDirtyGuard` + `requestClose` 를 `onClose`·취소 양쪽에)은 **여전히 옳다**. 그런데 **DS `Modal` 의 신규 latch**(`Modal.tsx:119,124` — `closingRef` 에 리셋 전무)가 dirty veto 경로에서 모달을 영구 고착시킨다. **'우리 코드가 그대로니 pass 유지' 로 처리하지 않았다** — 요구는 사용자가 겪는 동작이지 배선의 의도가 아니다
- [x] **`Toast` 를 같은 결함으로 잘못 묶지 않았다** — `exitingRef` 도 리셋되지 않지만 `onDismiss` 에 veto 경로가 없어 항상 언마운트된다. **vetoing 부모를 가진 Modal 만 깨진다**는 구분을 코드로 확인했다
- [x] **수정 방법을 지어내지 않았다** — 최소 수정은 veto 시 latch 리셋이나 `onClose` 가 `void` 반환이라 Modal 이 veto 를 알 방법이 현재 없다. 설계 결정이 필요하므로 §5 #3 에 **미정**으로 남겼다
- [x] **MOTION-01/02/03 의 낡은 관측을 코드로 반증해 교체했다** — '`Modal.css` 에 enter/exit 애니메이션이 없다'·'`ToggleSwitch.css` 에 `prefers-reduced-motion` 0건' 은 **둘 다 더 이상 사실이 아니다**(`Modal.css:126-168` · `ToggleSwitch.css:79-84`). 판정은 `종속` 유지 — 남의 판정을 대신 내리지 않았다
- [x] **직전 `4b805ad` 기준 기록(보존)**: 뒤집힌 P0 **2건**(STATE-01 · A11Y-11)을 pass 로 올렸다. **EXC-04 는 범위만 좁아졌으므로 gap 을 유지했다**(유령 저장 해소 ≠ 동시성 토큰 충족 — 이 구분을 흐리지 않았다). EXC-03·EXC-08 은 '앱에 기능이 없다' → '이 화면이 소비하지 않는다' 로 **사유만** 바꿨다
