...
| 판정 기준일 | **2026-07-17 · HEAD = `a5c2639`** (PR #22·#24·#26·#28·#30·#32·#34 머지 후). 직전 판정은 `4b805ad` 기준이었다. **이번 기준 갱신으로 P0 2건이 뒤집혔다 — gap 7 → 5**: **A11Y-11**(gap → **pass** — PR #30 이 `ImageUploadField` 의 required 를 **접근가능 이름**으로 잇는다: `requiredNameSuffix` `ImageUploadField.tsx:55` → `aria-label` `:250`. `aria-required` 가 아닌 이유는 드롭존이 `<button>` 이고 그 속성이 `role=button` 미지원이기 때문이다 — `:44-54`) · **MOTION-03**(gap → **종속** — `ToggleSwitch.css:79-84` 에 reduced-motion 게이트가 실재하게 됐다). **IA-02 · IA-04 · IA-13 · EXC-03 · EXC-04 는 gap 그대로다**(§2 · §5) |
...
> **규약의 예외 하나(명시)**: `상속` 항목이라도 **코드 대조로 위반을 직접 확인했고 그 표면이 이 화면에 실재하면 `gap` 으로 적고 범위를 '앱 전역'으로 표기한다.** `종속` 으로 덮으면 검증된 P0 미충족이 문서에서 사라진다(NFR-023 §1.1 과 동일 규칙). **`a5c2639` 기준 — 이 예외에 해당하는 행이 0건이 됐다**: 직전까지 A11Y-11 · MOTION-03 두 건이 여기 해당했으나 PR #30·#26 이 **둘 다 실제로 고쳤다**(`ImageUploadField.tsx:55,250` · `ToggleSwitch.css:79-84`).
...
| A11Y-11 | A11Y | 직접(+앱 전역) | **8개 중 7개 충족 — 1개 미충족.** ① **describedby 절 — 충족**: `aria-invalid` 를 쓰는 이 화면의 모든 컨트롤이 `aria-describedby` 로 이유를 연결한다 — `CaseStudyFormPage.tsx:130-133`(업종) · `:151-152`(일자) · `:167-168`(제목) · `:182-183`(고객사), `TextareaField.tsx:66-67`(과제·해결·성과 3개, 자체 배선), `ImageUploadField`/`ImageGalleryField`(`aria-invalid` 를 쓰지 않고 describedBy 만 — 짝 없는 위반이 아니다). 회귀는 `shared/a11y-guard.test.ts` 가 고정한다. ② **required 절 — F3a 가 대부분 닫았다**: `FormField.tsx:50-56` `withAriaRequired` 가 `required` 를 **런타임 `cloneElement`** 로 자식 컨트롤의 `aria-required` 에 주입한다(대상 판별 `:36-41`). 이 폼의 필수 8개 중 **7개가 주입 대상**이다 — 업종(자식이 DS `SelectField`, `:126`) · 일자(`:145` `input`) · 제목(`:159` `input`) · 고객사(`:174` `input`) · 과제·해결·성과(`TextareaField.tsx:64-65` 가 native `required`+`aria-required` 를 스스로 건다). ③ **남은 1개였던 대표 이미지 — PR #30 에서 닫혔다.** `PortfolioMediaFields.tsx:51-59` 의 `ImageUploadField required` 가 `FormField` 를 거치지 않는다는 사실은 그대로지만, DS 가 **`aria-required` 가 아니라 접근가능 이름으로** 이 축을 해결했다 — `requiredNameSuffix(required)`(정의 `ImageUploadField.tsx:55`, export `index.ts:2`)가 드롭존 `<button>` 의 `aria-label` 에 실린다(`:250`). 이 폼의 소비가 `label="대표 이미지" required` 이므로 이름이 **'대표 이미지 (필수) 이미지 업로드 — …'** 가 된다. **⚠ 이전 판정('required 미주입 잔여 결함')은 낡았을 뿐 아니라 요구하던 해법이 틀렸다** — 주석 `ImageUploadField.tsx:44-54` 가 근거를 못 박는다: AT 에 보이는 컨트롤이 `<button>`(드롭존)인데 **`aria-required` 는 `role=button` 이 지원하지 않는 속성**(ARIA 1.2 — textbox/checkbox/combobox/…)이라 얹으면 거짓 시맨틱이자 axe `aria-allowed-attr` 위반이고, 진짜 `<input type="file">` 은 `aria-hidden`+`tabIndex={-1}` 트리거라 그쪽에 줘도 **아무에게도 닿지 않는다**. 접근가능 이름이 유일하게 정직한 경로다. 시각 `*` 마커는 그대로 두고 이름에만 더했다. (`ImageGalleryField`(본문 이미지)는 `required` 를 넘기지 않으므로 suffix 가 빈 문자열이라 무관하다 — 다만 그쪽에도 같은 유틸이 배선돼 있다: `ImageGalleryField.tsx:237`, import `:14`. 뉘앙스 `:234-237,275-277` — suffix 는 **빈 드롭존에만** 붙고 '이미지 추가' 버튼은 생략한다. 그 시점엔 required('최소 한 장')가 이미 충족이기 때문이다) | ① `aria-describedby` 없는 `aria-invalid` grep = 0 → **통과**. ② 빈 폼 제출 후 입력의 `aria-describedby` === `role=alert <p>` id → **통과**. ③ **⚠ grep 으로 판정하지 말 것** — 주입이 런타임이라 소스에 리터럴이 없다(앱 전역 grep 1건은 무관한 수동 override). RTL 로 `getByLabelText` 7개의 `aria-required` === `'true'` → **7개 통과**. ④ 대표 이미지는 **`aria-required` 를 조회하지 말 것**(role=button 미지원 속성이라 영원히 null 이고, 그것이 의도다) — `getByRole('button', { name: /대표 이미지 \(필수\) 이미지 업로드/ })` 가 **매치되는지**로 판정한다 → **통과** | **pass** |
...
| IA-13 | IA | 직접 | **미충족.** list query state(업종 필터)가 **컴포넌트 `useState` 에만 있다** — `CaseStudyListPage.tsx:56` `const [filter, setFilter] = useState<CaseFilter>(CASE_FILTER_ALL)`. 파일에 `useSearchParams`·`useListState` import 0건. `parseFilter`(`:106`)로 값을 타입 안전하게 좁히기는 하지만 **그 값이 URL 로 나가지 않는다** — 안전한 값을 URL 없이 들고 있을 뿐이다. **F3b 가 `shared/crud/useListState`(URL 단일 원천)를 34개 화면에 롤아웃했고 같은 섹션의 `support/{downloads,replies,tickets}` 도 소비처가 됐지만**(`DownloadListPage.tsx:87` · `RepliesPage.tsx:76` · `TicketListPage.tsx:129`) **`pages/portfolio/**` 3화면은 전부 그 밖에 남았다.** 정렬은 서버 고정이라 URL 대상이 아니고, page·keyword 는 표면 자체가 없다 | 업종을 '금융'으로 바꾼 뒤 ① F5 → **'전체 업종'으로 되돌아간다** ② 행을 눌러 수정 폼에 갔다가 Back → **필터가 풀린 목록에 착지한다** ③ 주소창 URL 을 복사해 새 탭에 붙여넣기 → **필터가 재현되지 않는다**(URL 에 `?` 가 아예 붙지 않는다). `grep -rn 'useSearchParams\|useListState' apps/admin/src/pages/portfolio/` = **0**(앱 전체로는 34개 화면 — `a5c2639` 재확인). 대조: `/support/downloads` 에서 같은 조작을 하면 URL 에 `?category=…` 가 붙고 Back·F5 를 견딘다 | **gap** |
...
| 판정 | 건수 | 요구 ID |
|---|---|---|
| `pass` | **13** | STATE-01 · **STATE-02** · STATE-04 · TOKEN-01 · **A11Y-11** · FEEDBACK-02 · FEEDBACK-04 · IA-01 · IA-05 · EXC-01 · EXC-02 · EXC-08 · EXC-09 |
...
| `gap` | **5** | IA-02 · IA-04 · IA-13 · EXC-03 · EXC-04 |

**검산: 13 + 9 + 3 + 5 = 30** ✓ (STATE 3 · TOKEN 5 · COMP 1 · FEEDBACK 3 · A11Y 4 · MOTION 3 · IA 5 · EXC 6 = 30)

> **`a5c2639` 기준 재판정 결과: P0 2건이 뒤집혔다 — gap 7 → 5.** 둘 다 **DS 가 실제로 고쳤다**:
...
> - **MOTION-03 gap → 종속** — quality-bar 가 appliesTo 로 지목한 `ToggleSwitch` reduced-motion 부재가 **해소됐다**(`ToggleSwitch.css:79-84`). 이전 판정의 근거였던 '파일 전체에 해당 미디어 쿼리 0건' 은 이제 거짓이다. ⚠ quality-bar 의 MOTION-03 요구문은 **요구 정본이라 그대로 둔다**.
>
> **직전 `4b805ad` 기준 기록(보존)**: 뒤집힌 P0 0건. **A11Y-11 의 범위를 8→1 로 좁혔을 뿐**이었고(남은 것은 `ImageUploadField` 하나) 그때 '좁혀진 gap 도 gap' 으로 유지한 판단은 옳았다 — **이번에 그 마지막 하나가 실제로 닫혀서** pass 가 됐다. **P1 2건은 그때 pass 가 됐다**(ERP-13 · EXC-08 의 멱등키 절 — §3 참조). EXC-03·IA-13 은 사유가 '앱 전역 미비' → **'이 화면의 미소비'** 로 바뀌었다(공용 훅과 선례가 각각 7곳·34곳에 실재한다).
>
> **NFR-023 대비**: pass 12 → **13**, gap 6 → **5**. 차이는 여전히 **STATE-02 한 건**(§1.2) — 업종이 소스 상수라 표면 없는 read query 가 존재하지 않는다. **EXC-04 도 두 문서의 결이 다르다**: 이 화면은 `createCrudAdapter` 를 쓰므로 F2 때부터 409 를 냈고, 포트폴리오(`createStoreAdapter`)는 F3b 에 와서야 같아졌다 — **이제 두 팩토리가 같은 계약을 갖는다**(`crud.ts:126-128`·`:219-221`). 남은 EXC-04 gap 사유는 양쪽 모두 **동시성 토큰 하나**다.
>
> **P0 gap 5건 = quality-bar '배치 실패' 사유.** 5건 중 **이 화면 고유는 3건**(IA-13 · EXC-03 · EXC-04)이고 — IA-13·EXC-03 은 공용 훅을 소비만 하면 되고, EXC-04 는 도메인 타입에 `version` 이 필요하다 — **2건은 앱 전역 범위**(IA-02 · IA-04)로 이 화면만 고쳐서는 해소되지 않는다. **DS 범위였던 2건(A11Y-11 · MOTION-03)은 DS 가 고쳐서 사라졌다.** §5 의 '범위' 열이 이를 가른다.
...
- [x] **판정 기준일을 `2026-07-17 · HEAD = a5c2639`(PR #22·#24·#26·#28·#30·#32·#34) 로 갱신하고 P0 30건을 전수 재확인했다.** 뒤집힌 P0 **2건 — gap 7 → 5**: **A11Y-11** gap→**pass**(`ImageUploadField.tsx:55,250` — 마지막 하나가 닫혔다) · **MOTION-03** gap→**종속**(`ToggleSwitch.css:79-84` — 게이트 실재). §2.1·§5 #6·#7 을 함께 갱신했다
- [x] **직전 기준에서 '좁혀진 gap 도 gap' 으로 버틴 판단이 옳았음을 확인했다** — `4b805ad` 때 A11Y-11 을 8→1 로 좁히고도 pass 로 올리지 않았고, **이번에 그 마지막 하나가 실제로 닫혀서** pass 가 됐다
- [x] **낡은 주장 2종을 코드로 반증해 지웠다**: ① 「`ToggleSwitch.css` 파일 전체에 `prefers-reduced-motion` 0건」 → **틀렸다**(`:79-84`, grep = 1) ② 「`Modal.css` 에 enter/exit 선언 없음 — DS 미도입으로 보인다」 → **틀렸다**(`Modal.css:126-168`). 모션은 실재하되 **CSS-only** 다(Motion/framer-motion 은 여전히 미도입)
- [x] **A11Y-11 이 '내가 요구하던 방식'으로 고쳐지지 않았다는 이유로 gap 을 유지하지 않았다** — DS 는 `aria-required` 대신 **접근가능 이름**을 골랐고, `aria-required` 가 `role=button` 미지원 속성이라는 점에서 **그쪽이 정확하다**. 요구는 'required 가 AT 에 닿을 것' 이지 '특정 속성을 쓸 것' 이 아니다
- [x] **직전 `4b805ad` 기준 기록(보존)**: P0 판정이 뒤집힌 것은 **0건**(12/8/3/7). **범위가 좁아진 P0 1건**(A11Y-11 8→1)은 gap 을 유지했다. **P1 2건은 pass 로 전환**(ERP-13 · EXC-08 멱등키 절)했고 §3·§5 를 함께 갱신했다
...
- [x] §1.1 표기 규약의 예외(상속 항목의 검증된 위반을 gap 으로 적는 규칙)를 명시했다. **`a5c2639` 기준 해당 건수는 0 이다** — 그 예외로 붙잡아 뒀던 2건(A11Y-11 · MOTION-03)을 DS 가 실제로 고쳤다. 예외 규칙 자체는 유지한다
