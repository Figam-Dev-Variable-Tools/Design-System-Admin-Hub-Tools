...
| # | 내용 | 이관 대상 |
|---|---|---|
| 1 | 401 감지 → `/login?returnUrl=` 처리가 프론트에 없다 | A11 · A40 |
| 2 | 프론트 요청 타임아웃 상한 없음(권고: 조회·쓰기 10초) | A11 · A40 |
| 3 | 수정 동시성 — 낙관적 잠금(ETag/version) 미도입, 마지막 쓰기 승리. 두 관리자가 같은 공지를 동시 편집하면 늦은 저장이 이긴다 | A63 · A11 |
| 4 | 등록 전송 재시도로 중복 생성 가능(멱등키 없음). 현재 규모에서 허용 | A63 |
| 5 | `keyword` 100자 상한이 프론트에 없다 — 초과 시 400 이 조회 실패 배너로 표시된다 | A11 |
