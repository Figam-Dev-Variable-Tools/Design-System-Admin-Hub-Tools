...
| 10 | **조회 실패가 화면 전체를 대체해 컨트롤까지 지운다** — `ScheduleCalendarPage.tsx:168-181` 이 이른 반환으로 뷰 토글·기간 이동을 함께 없앤다. 실패 상태에서 운영자가 할 수 있는 일은 '다시 시도' 하나뿐이고, 보던 기간이 어디였는지도 화면에서 사라진다 | A11 change_request |
