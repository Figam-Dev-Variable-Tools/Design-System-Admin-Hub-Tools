...
| MOTION-01 | MOTION | 상속 | 이 화면의 Modal 표면: 삭제 확인 · 충돌 · discard 확인 — **이 섹션에서 Modal 을 가장 많이 여는 화면이다**. enter/exit transition 은 DS `Modal` organism 소유 | Modal 소유 문서 판정 (현재 `packages/ui/src/organisms/Modal/` 에 Motion/AnimatePresence 0건 — DS 측 미구현) | **종속** |
| MOTION-02 | MOTION | 상속 | 이 화면의 toast 표면: 저장·삭제 성공 토스트 3종. exit 애니메이션은 `ToastProvider` 소유 | ToastProvider 소유 문서 판정 | **종속** |
| MOTION-03 | MOTION | 상속 | 이 화면은 **자체 animation/transition 을 선언하지 않는다**(두 파일에 `transition`·`animation` 0건). reduced-motion 게이트 대상은 전부 DS(Modal·Toast·skeleton pulse·row hover) | 전역 Motion config / DS CSS 소유 문서 판정 | **종속** |
...
>   ⚠ **같은 F3a 를 받고도 NFR-037(예약 폼)은 여전히 gap 이다** — 그쪽은 '이용 시간'을 래퍼 `<div>` 로 감싸 두 입력을 넣었기에 주입 대상에서 제외된다. **이 폼이 pass 인 것은 운이 아니라 희망 날짜/시각을 각자의 `FormField` 로 나눈 구조 덕이다.**
...
- [x] **A11Y-11 의 pass 를 같은 섹션 NFR-037 의 gap 과 대조**했다 — 같은 `FormField` 수정을 받고도 예약 폼은 '이용 시간'이 래퍼 `div` 자식이라 주입에서 제외된다. **구조가 판정을 갈랐다는 사실을 문서에 남겼다**
