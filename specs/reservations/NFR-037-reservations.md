...
| A11Y-11 | A11Y | 직접 | **부분 미충족(gap 유지 — 그러나 사유가 완전히 바뀌었다. 범위가 7필드 → 1필드로 줄었다).** ① **짝 없는 `aria-invalid` = 0건**(유지) — 고객명·연락처·날짜·시각(시작/종료)·인원·예약금 전부 `aria-invalid={errors.x !== undefined}` 와 `aria-describedby={… ? errorIdOf('rsv-…') : undefined}` 를 짝지어 붙인다(`ReservationFormPage.tsx:212-215, 233-236, 249-250, 264-265, 275-276, 307-308, 334-335`). 시작/종료 시각이 같은 오류 id 를 가리키는 것은 **의도**다 — 두 입력이 하나의 `FormField`(htmlFor="rsv-start")와 하나의 오류 `<p>` 를 공유한다. ② **required 의 AT 경로가 F3a 로 생겼다 — 단 한 필드만 빼고.** `FormField.tsx:50-56` `withAriaRequired()` 가 `required` 를 **런타임 `cloneElement` 로 자식 컨트롤의 `aria-required` 에 주입**한다. 주입 대상은 `isRequirableChild`(`:36-41`)가 고르며 **네이티브 `input`/`select`/`textarea` 와 DS `SelectField` 뿐**이다. 이 화면의 required 7필드를 자식 타입으로 전수 확인한 결과 **6건 주입 · 1건 미주입**이다: 고객명(`input:204` ✔) · 연락처(`input:225` ✔) · 방문 날짜(`input:243` ✔) · 자원 배정(`SelectField:289` ✔) · 예약 인원(`input:299` ✔) · 상태(`SelectField:342` ✔) / **이용 시간(`:255`) ✕ — 자식이 `<div style={rowStyle}>`(`:256`)이고 그 안에 `<input type="time">` 2개(시작 `:257` · 종료 `:268`)가 들어 있다.** `div` 는 requirable 이 아니라(래퍼에 `aria-required` 를 얹으면 **거짓 시맨틱** — `FormField.tsx:32-34,20-21` 이 밝히는 **의도된 설계**다) 주입이 일어나지 않는다. ⚠ **즉 이것은 `FormField` 의 결함이 아니라 이 호출부의 구조가 계약 밖이라는 사실이다** — 하나의 라벨이 두 컨트롤을 덮는 복합 필드에는 주입할 단일 대상이 없다. 앱 소스의 `aria-required` grep 이 1건뿐인 것으로 판정하면 안 된다(주입은 런타임이다) | ① `/reservations/new` 에서 빈 폼 제출 → 각 입력의 `aria-describedby` 가 `role="alert"` `<p>` 의 id 와 일치(RTL 로 검증 가능). ② `/reservations/new` 를 RTL 로 렌더 → **고객명·연락처·방문 날짜·자원 배정·예약 인원·상태 6개 컨트롤에 `aria-required="true"` 가 실재한다**(주입 결과). **그러나 `#rsv-start`·`#rsv-end` 에는 없다** — 래퍼 `div` 가 주입을 삼켰고 그 `div` 자신도 받지 못했다. **'이용 시간'의 필수 여부는 AT 에 영원히 닿지 않는다**: 별표는 `aria-hidden`(`FormField.tsx:96-100`)이라 스크린리더 사용자는 이 필드가 필수임을 제출 전에 알 수 없다. **해소 방향(미정)**: 시작/종료를 각자의 `FormField` 로 쪼개거나, 호출부가 두 `<input>` 에 `aria-required` 를 **직접** 주면 된다(`FormField.tsx:54` 가 호출부 값을 우선한다) — 어느 쪽인지는 A11/A41 이 정한다 | **gap** |
...
| 판정 | 건수 | 요구 ID |
|---|---|---|
| `pass` | **8** | STATE-01 · STATE-02 · STATE-04 · TOKEN-01 · **COMP-10** · IA-01 · IA-05 · **IA-13** |
...
> **P0 gap 5건 — quality-bar §How to use 기준 '배치 실패' 사유다.** 전부 §5 로 이관한다.
>
...
| 1 | **A11Y-11** | **P0** | **필수 7필드 중 6건은 해소됐다** — `FormField.tsx:50-56` `withAriaRequired` 가 `aria-required` 를 자식 컨트롤에 런타임 주입한다. **남은 1건: '이용 시간'(`ReservationFormPage.tsx:255`)** — 자식이 래퍼 `<div>`(`:256`)라 `isRequirableChild`(`FormField.tsx:36-41`)가 거부한다(거짓 시맨틱 방지 — **의도된 설계**). 그 필드의 필수 여부가 AT 에 닿지 않는다. **`FormField` 의 결함이 아니라 복합 필드 호출부의 구조 문제다.** 해소 방향(시작/종료를 각자의 `FormField` 로 쪼개기 vs 호출부가 `aria-required` 직접 주기)은 **미정** | 이 화면(호출부) | A11 · A41 |
