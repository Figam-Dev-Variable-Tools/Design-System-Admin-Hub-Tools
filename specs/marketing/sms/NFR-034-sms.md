...
| 판정 기준일 | **2026-07-17 · `HEAD = 4b805ad`** (F3a·F3b·통합 머지 후 — PR #8·#11·#12·#14·#16). 이전 판정은 F2(`3cd3078`) 기준이었고 **COMP-10 · IA-13 이 pass 로 뒤집혔다**. **A11Y-11 · ERP-09 는 gap 그대로다** — 전자는 `SegmentPicker` 가 `FormField` 를 거치지 않아 F3a 의 `aria-required` 주입이 닿지 않고, 후자는 예약 시각 해석이 여전히 브라우저 로컬이다(§2 · §3 · §5). **E2E 미실행 — 판정 근거는 전부 코드 대조다** |
...
| TOKEN-01 | TOKEN | 직접 | 이 화면 전용 모듈의 모든 시각 값이 `var(--tds-*)` 다 — `SmsListPage.tsx` · `SmsFormPage.tsx` · `PhoneMessagePreview.tsx` · `SmsMessageCard.tsx` · `_shared/{SegmentPicker,VariableInsertBar}.tsx` 전수 grep 에서 primitive 밖 hex 0건 · px 리터럴 0건 · border/outline 키워드 0건 | `grep -rnE "#[0-9a-fA-F]{3,8}\b\|[0-9]+px\|(outline\|border):\s*(thin\|medium\|thick)" apps/admin/src/pages/marketing/{sms,_shared}` → 0건(`#{변수}` 토큰 제외). ESLint/stylelint 0 warning | pass |
...
| TOKEN-03 | TOKEN | 상속 | 이 화면이 렌더하는 애니메이션 표면은 DS 소유뿐이다 — Toast(삭제 성공·저장 성공)와 ToggleSwitch transition(`ToggleSwitch.css:32,56`). easing 토큰을 이 화면 코드가 직접 참조하는 곳은 0건이다 | tokens codegen · `Toast.css` 소유 문서의 판정을 따른다 | 종속 |
...
| A11Y-11 | A11Y | **직접** | **부분 충족.** 짝 없는 `aria-invalid` 는 0건이다 — 발송명(`SmsFormPage.tsx:286-287`)·예약 일시(`:382-385`)가 `aria-invalid` + `aria-describedby={errorIdOf(...)}` 를 함께 싣고, 본문은 `TextareaField` 가 자동 배선하며(`TextareaField.tsx`), 발신번호는 `SelectField isInvalid`(`:301`)가 담당한다. **그러나 `SegmentPicker` 가 계약을 지키지 않는다 — 2026-07-17 재확인, F3a 가 이 구멍을 덮지 못한다**: F3a 가 `FormField.tsx:50-56` `withAriaRequired` 로 `required`→`aria-required` 를 런타임 주입하게 만들었고 이 폼의 **`FormField` 를 거치는 required 필드는 전부 주입된다**(발송명 `<input>` `:278` · 발신번호 `<SelectField>` `:298` · 발송 방식 `<SelectField>` `:364` · 예약 일시 `<input>` `:376`; 본문은 `TextareaField.tsx:64-65` 가, 그 밖은 `SelectField.tsx:60` 이 자기 것을 낸다). **그러나 `SegmentPicker` 는 `FormField` 를 아예 거치지 않는다** — `SegmentPicker.tsx:109-112` 가 bare `<span style={fieldLabelStyle}>` + `<span aria-hidden="true"> *</span>` 로 라벨과 마커를 손수 그린다. 그래서 ① `required` 가 **AT 에 영영 닿지 않고**(마커가 `aria-hidden` 이므로) ② 체크박스 그룹에 `fieldset`/`legend` 도 `role="group"` 도 없으며 ③ 오류 `<p role="alert">`(`:137-140`)가 어느 컨트롤과도 `aria-describedby` 로 연결되지 않는다. 같은 결로 `ToggleSwitch` 는 `aria-describedby` prop 자체를 노출하지 않는다(`ToggleSwitch.tsx:19-38`) | ① `grep -rn "aria-invalid" apps/admin/src/pages/marketing/sms` → 3건 전부 `aria-describedby` 동반(pass). ② `/marketing/sms/new` 에서 세그먼트 미선택으로 제출 → 오류가 뜨지만 RTL 로 체크박스의 `aria-describedby` 를 조회하면 `null`, `aria-required` 도 없다 → **요구의 'required 는 native required/aria-required 로 노출' 절 미충족**. ③ **`aria-required` 를 앱 소스에서 grep 하지 말 것** — 주입이 런타임 `cloneElement` 라 grep 은 전 앱 1건(`members/CreateGroupModal.tsx:192` 수동 override)만 잡는다. 렌더 후 `document.querySelectorAll('[aria-required="true"]')` 로 확인해야 한다 | **gap** |
...
| MOTION-02 | MOTION | 상속 | **충족하지 않는다.** 이 화면이 토스트를 띄운다(삭제·저장 성공). `ToastProvider` 에 `AnimatePresence` 가 0건이라 dismiss 가 즉시 filter-out 으로 끊긴다 — entrance 만 CSS 로 있고 exit 애니메이션이 없다 | `grep -rn "AnimatePresence" apps/admin/src/shared/ui/ToastProvider.tsx` → 0건. SMS 발송을 저장 → 성공 토스트가 자동 소멸할 때 fade/translate 없이 사라진다 | **gap** (범위: 앱 전역 · DS 소유) |
| MOTION-03 | MOTION | 상속 | **충족하지 않는다.** 이 화면이 `ToggleSwitch` 를 **2개** 렌더한다 — 광고성 여부(`SmsFormPage.tsx:329-338`) · 이미지 첨부(`SmsMessageCard.tsx:110-119`). `ToggleSwitch.css` 가 `transition: background-color`(`:32`)와 **`transition: transform`(`:56`)** 를 걸면서 `@media (prefers-reduced-motion: reduce)` off 를 두지 않는다 — quality-bar 가 MOTION-03 본문에서 직접 지목한 위반이 그대로 남아 있다. 전역 reduced-motion 게이트(`MotionConfig`/`useReducedMotion`)도 없다(Motion 미도입) | `grep -n "prefers-reduced-motion" packages/ui/src/atoms/ToggleSwitch/ToggleSwitch.css` → **0건**. OS/DevTools 에서 `prefers-reduced-motion: reduce` 를 켜고 광고성 토글을 조작 → handle 이 여전히 transform transition 으로 미끄러진다 | **gap** (범위: 앱 전역 · DS 소유) |
...
**검산**: pass 11 + 종속 10 + n-a 2 + gap 7 = **30** ✓

> **2026-07-17 · `4b805ad` 기준 변경**: F2 판정의 gap 9건 중 **2건이 pass 로 뒤집혔다** — **COMP-10**(F3b 가 `useListState` → `useDebouncedSearch` 를 이 화면에 배선) · **IA-13**(같은 훅이 조회 상태를 URL 로 옮겼다). **IA-02 · EXC-03 은 gap 을 유지하되 사유가 바뀌었다**(브랜치 폴백 ✕ → h1 이중·행위 미반영 / 소비자 0 ✕ → 7곳 있으나 marketing 미배선). **A11Y-11 은 gap 그대로다** — F3a 가 `FormField.tsx:50-56` 에 `aria-required` 런타임 주입을 넣었지만 **`SegmentPicker` 는 `FormField` 를 거치지 않아**(`SegmentPicker.tsx:109-113` 이 bare `<span>` + 수동 `*`) 그 주입이 닿지 않는다. **MOTION-01/02/03 도 그대로다** — Motion 라이브러리 미도입.
>
> **⚠ P0 gap 7건 — quality-bar §How to use 기준 '배치 실패' 사유다.** 성격이 갈린다: **이 화면/섹션 소유 4건**(A11Y-11 — `_shared/SegmentPicker.tsx` · IA-02 · IA-04 · EXC-03)과 **앱 전역/DS 소유 3건**(MOTION-01/02/03 — Motion 라이브러리 미도입). 후자는 이 화면만 고쳐서 닫을 수 없다. §5 가 범위를 갈라 이관한다.
...
| A11Y-16 | P1 | 이 화면의 신규 표면이 계약을 대체로 지킨다 — `PhoneMessagePreview` `aria-label`(`:106`) · `VariableInsertBar` 칩 `aria-label`(`:65`) + `tds-ui-focusable` · `ToggleSwitch` `role="switch"`. **단 `SegmentPicker` 가 그룹 semantics(fieldset/legend·aria-required·error 연결) 없이 `<ul>` + bare `<span>` 라벨이다** | axe 로 `/marketing/sms/new` 스캔 | 부분 pass(→ §5 #3) |
...
| # | 요구 ID | P | 내용 | 범위 | 이관 |
|---|---|---|---|---|---|
| 1 | ~~**IA-13**~~ | ~~P0~~ | **해소 (F3b · 2026-07-17)** — `SmsListPage.tsx:117` 이 공용 `useListState` 를 소비해 `?status=`·`?q=` 를 URL 이 소유한다(`useListState.ts:87-99,125`). F5·Back·링크 공유로 복원된다. `useListState` 소비 화면은 이제 **37곳**(마케팅 6화면 전부 포함) | — | **닫힘** |
...
| 3 | **A11Y-11** | **P0** | **gap 유지 (2026-07-17 재확인).** `SegmentPicker` 의 required 표식이 `aria-hidden` `<span>` 이고(`SegmentPicker.tsx:109-112`) `aria-required` 가 없으며, 체크박스 그룹에 `fieldset`/`legend`·`role="group"` 도 없고, 오류 `<p role="alert">`(`:137-140`)가 체크박스와 `aria-describedby` 로 연결되지 않는다. `ToggleSwitch` 도 `aria-describedby` prop 미노출. **⚠ F3a 가 이 구멍을 덮지 못한다** — `FormField.tsx:50-56` `withAriaRequired` 의 런타임 `cloneElement` 주입은 `FormField` 를 **거치는** 자식(`input`/`select`/`textarea`/`SelectField`)에만 적용되는데, `SegmentPicker` 는 `FormField` 를 아예 쓰지 않는다. **이 화면의 나머지 required 필드는 전부 주입된다** — 결함은 이 컴포넌트 하나로 국소화됐다 | **`_shared/SegmentPicker.tsx` 1곳**(SMS·이메일·뉴스레터 3화면이 함께 쓴다) + DS(`ToggleSwitch`) | A11 · A40 (FS-034 §7 #3) |
...
| 5 | **IA-04** | **P0** | Pagination 이 없다 — `marketing/**` 전역 `Pagination`·`pageSize` 참조 0건. 캠페인 전량이 한 표에 그려진다. DS `Pagination` 은 F3a 에서 범위/page-size 를 **opt-in**(`Pagination.tsx:112` `pageSize > 0`)으로 열었으나 실제 opt-in 소비자는 `stats/*`·`logs` 뿐이고, `useListState` 가 주는 `page`·`clampPage`(`useListState.ts:89,217-223`)도 이 화면이 쓰지 않는다 | 이 화면 + `CrudListShell` | A11 (FS-034 §7 #9) |
| 6 | **EXC-03** | **P0** | **gap 유지 · 범위 축소.** 쓰기 권한 게이팅이 없다 — 등록·수정·삭제 버튼이 권한과 무관하게 렌더. **`useRouteWritePermissions`(`RequirePermission.tsx:45`) 소비처는 0 → 7곳**(products 3 · settings 4)이 됐다. 훅·선례는 완비돼 있고 **`pages/marketing/**` 이 아직 배선되지 않았을 뿐**이다 | **이 섹션**(앱 전역 아님 — 선례 7곳 존재) | A11 · A63 (FS-034 §7 #10 · BE-034 §7.12 #7) |
| 7 | **MOTION-01** | **P0** | Modal enter/exit 모션 없음 — Motion 라이브러리 미도입(`package.json` 에 `framer-motion`/`motion` 0건) | **앱 전역 · DS 소유** | A40 (DS) |
| 8 | **MOTION-02** | **P0** | Toast exit 모션 없음 — `ToastProvider` 에 `AnimatePresence` 0건 | **앱 전역 · DS 소유** | A40 (DS) |
| 9 | **MOTION-03** | **P0** | `ToggleSwitch.css:32,56` 의 transition 에 `prefers-reduced-motion` off 없음. 이 화면이 ToggleSwitch 2개를 렌더 | **앱 전역 · DS 소유** | A40 (DS) |
| 10 | COMP-12 | P2 | **본문 카운터(글자수)와 검증 상한(바이트)의 기준이 다르다** — 카운터 'N/2000'(코드포인트) vs 검증 2,000 byte(`validation.ts:65`). 한글은 1,000자에서 걸리는데 카운터는 절반 남았다고 말한다. 바이트 힌트(`SmsMessageCard.tsx:98-101`)가 완화하나 두 숫자가 서로 다른 한도를 말한다. **counting 기준을 byte 로 정의하고 서버 강제와 일치시켜야 한다** | 이 화면 | A11 (FS-034 §7 #5 · BE-034 §4 EP-03) |
| 11 | IA-10 | P2 | 2-col preview shell 이 손조립이고 `EmailFormPage` 가 같은 상수를 복제한다(`SmsFormPage.tsx:88-93` ≡ `EmailFormPage.tsx:82-87`) | marketing(+popup/banner/quote) | A11 |
| 12 | ERP-09 | P2 | 야간·과거 판정이 브라우저 로컬 TZ 기준(`messaging.ts:258-262` `isNightAt` → `getHours()` · `validation.ts:17-21` `isPastLocal`). 법정 기준 Asia/Seoul 이 계약에 없다. **F3b 의 KST 표시 고정(`format.ts:63,76-85`)이 이 축을 덮지 못했다 — 오히려 표시(KST)와 검증(로컬)이 갈려 불일치가 선명해졌다.** 같은 결함이 이메일·뉴스레터에도 그대로 있다(`_shared/messaging.ts` 공유 + 세 `validation.ts` 의 `isPastLocal` 사본 3벌) | 이 화면 + `_shared`(이메일·뉴스레터 공유) | A63 · A11 (FS-034 §7 #7 · BE-034 §7.5) |
| 13 | EXC-18 / EXC-10 | P1 | 일괄 삭제에 Shift-range · 강화 confirm · 진행률 · 실행 중 취소 · 실패 항목 식별이 없다(건수만) | 이 화면 + `useCrudList` | A11 (FS-034 §7 #9) |
| 14 | EXC-05 / EXC-11 / EXC-19 | P1 | 프론트 타임아웃 상한 · 오프라인 감지 · 세션 만료 시 폼 draft 보존이 없다(`AbortSignal.timeout` · `navigator.onLine` 앱 전체 0건) | 앱 전역 | A11 · A40 (FS-034 §7 #15) |
| 15 | IA-14 | P1 | 반응형 계약(최소 폭·sidebar collapse·표 가로 스크롤·touch-target)이 선언되지 않았다 | 앱 전역 | A40 |
| 16 | STATE-05 | P1 | 진짜 비어있음 분기에 생성 CTA 가 없다 — `empty.createAction` 미전달(`SmsListPage.tsx:161-166`) | 이 화면 | A11 |
| 17 | (기능) | — | **발송 트리거 UI 가 없다** — `/send` 심(`data-source.ts:71`)에 호출부 0건. 저장된 캠페인을 보낼 방법이 이 화면에 없다. 구현 시 **멱등키 필수**(BE-034 §7.3). 발송 UI 가 없으므로 **중복 발송 경합은 현재 성립하지 않는다** — 트리거가 붙는 순간 EXC-08 의 진짜 대상이 된다 | 이 화면 | A11 · A63 (FS-034 §7 #8 · BE-034 §7.12 #1) |
| 18 | (감사) | — | 발송 감사 필드(작성자·실행자·실행 시각·동의 근거)가 도메인 타입에 없고, 발송 이력이 캠페인 hard-delete 로 함께 사라진다 — **법적 증빙 요구 미충족**(§4.3) | 이 화면 + 서버 | A63 (BE-034 §7.9) |
...
| grep 판정 | `Pagination`(IA-04) · `useListState`/`useDebouncedSearch`(IA-13·COMP-10) · `useRouteWritePermissions`(EXC-03) · `AnimatePresence`/`framer-motion`(MOTION-01/02) · `prefers-reduced-motion`(MOTION-03) · hex/px/border-keyword(TOKEN-01) |
