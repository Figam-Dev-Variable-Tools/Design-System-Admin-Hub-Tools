...
프론트 수정 이관: A11 change_request — 승인상태 select·반려 사유 입력을 **읽기 전용 표시**로 바꾸고(현재 상태 배지 + 카카오 반려 사유 표시), 그 자리에 **'심사 제출' 버튼**(EP-05 호출)을 둔다. **그 수정이 들어와도 이 서버 판정은 그대로 남는다** — 프론트는 UX 이고 서버가 정본이다.
...
프론트 이관: A11 — 본문 카운터를 채널에 따라 문자/바이트로 바꾸고, 한도 초과를 **검증으로 막는다**(지금은 힌트만 뜬다). quality-bar COMP-12 가 'counting 기준(code point vs byte)을 정의하고 서버 강제와 일치시킨다' 를 요구한다.
...
**한계 기록**: 실패한 **id 를 돌려주지 않아** 재시도가 성공분까지 재실행한다(FS-036 §7 #11, quality-bar EXC-10). 선택 건수 상한·동시성 제한이 없어 레이트리밋(분당 60)에 걸리면 실패 건수만 늘어난다(EXC-18). A11 이관.
...
| # | 내용 | 이관 |
|---|---|---|
| 1 | **승인상태·반려사유를 폼이 직접 지정한다** — 카카오 심사 우회. 승인 후 본문 변경·채널 전환 잔여값·반려 사유 자작 3개 부수 경로 포함(§7.1) | **A11 change_request · A63(최우선)** |
...
| 2' | **낙관적 동시성 토큰이 없다** — 위 409 는 '존재 여부' 기반이지 `version`/ETag 가 아니다. **둘 다 존재하는 동시 편집은 last-write-wins** 다. `MessageTemplate`(`_shared/messaging.ts:137-147`)에 `version` 이 없고 `updatedAt`(`:146`)은 표시값일 뿐이다. EP-04 의 `If-Match`(§4)를 실제로 요구하려면 서버가 `version`/ETag 를 응답에 실어야 한다(§7.5 판정 2) | **A63 · A41(`_shared/messaging.ts`·어댑터)** |
| 2'' | **삭제 멱등 계약이 픽스처와 갈린다** — 서버는 이미 삭제된 id 에 **204**(멱등)로 답하기로 돼 있는데 픽스처는 **409** 를 던진다(`crud.ts:232-234`). 연동 시 일괄 삭제의 '실패 건수' 집계가 달라진다(현재 재현 불가 — §7.5 · §4 EP-07) | A63 · A11 |
| 3 | **심사 제출 계약 미정** — 심만 있고 호출부 0건. 심사 결과 수신 방식(웹훅 vs 폴링)이 미정(§4 EP-05) | A01 · A63 |
| 4 | **발송 시점 승인 재검증 방식 미정** — 값 복사 구조라 발송 요청에 템플릿 id 가 없다. 발송 계약 확정 시 함께 정한다(§7.4) | A63 |
| 5 | 길이 기준이 문자/바이트로 갈려 있고 SMS 본문이 LMS 한도를 넘겨도 저장된다. 알림톡 실제 상한 미정(§7.8) | A11 · A63 |
| 6 | 목록 정렬 계약 신설(`updatedAt` 내림차순) + 페이징·서버 검색(§4 EP-01 · §7.11) | A11 · A63 |
| 7 | 템플릿명 전역 유니크(409) — 프론트에 중복 검사 없음(§7.7) | A63 |
| 8 | 일괄 삭제가 실패 id 를 돌려주지 않고 선택 상한·동시성 제한이 없다(§7.9) | A11 |
| 9 | SMS/알림톡 본문의 URL 스미싱 방지(발신 도메인 화이트리스트) 필요 여부 미정(§7.2-3) | A63 |
| 10 | 401 감지 시 작성 중 폼이 사라진다 · 프론트 타임아웃 상한 없음 · 오프라인 감지 없음 | A40 · A63 |
| 11 | 쓰기 권한 게이팅 미배선 — **승인상태 select(`TemplateFormPage.tsx:202-216`)도 권한 축으로 막히지 않는다**(§2 · §7.1). **`useRouteWritePermissions`(`RequirePermission.tsx:45`) 소비처는 F3b 이후 0 → 7곳**(products 3 · settings 4)이 됐으나 **`pages/marketing/**` 소비는 0건**이다 — 훅·선례는 완비돼 있고 이 섹션 배선만 남았다 | A11 · A40 |
| 12 | 세그먼트·발신자와 마찬가지로 `listSendableTemplates` 가 **동기 함수**라 발송 폼에 로딩·실패 경로가 없다 — 비동기 재구조화 필요(§4 EP-06 · BE-033 §7.6 과 같은 묶음) | A63 |
