...
| 판정 시점 | **2026-07-17 · `HEAD = 4b805ad`** (F3a·F3b·통합 머지 후 — PR #8·#11·#12·#14·#16) 코드 대조. 이전 판정은 F2(`3cd3078`) 기준이었고 **COMP-10 · IA-13 · EXC-04 가 pass 로 뒤집혔다**(§2 · §2.1 · §5). 특히 **EXC-04 는 F3b 가 `createStoreAdapter` 에 존재 검사 + 409 를 넣어 유령 저장이 해소된 것**이다 — 이 문서가 'NFR-033 과 갈린 쪽' 이라고 적었던 그 차이가 사라졌다. **E2E 미실행 — 판정 근거는 전부 코드 대조다** |
...
| MOTION-01 | MOTION | 상속 | 이 화면의 `Modal` 표면 — 삭제 확인·충돌·미저장 가드 다이얼로그 — 이 DS Modal 의 enter/exit transition 계약을 상속한다 | DS 소유 문서 판정. 이 화면에서는 삭제 다이얼로그 open/close 에 backdrop fade + dialog scale 이 보이고 reduced-motion 에서 즉시 처리되는지만 확인 | 종속 |
...
| MOTION-03 | MOTION | 상속 | 이 화면은 자체 Motion 을 도입하지 않는다 — 모든 움직임(Modal·Toast·스켈레톤 pulse)이 DS 소유다 | DS 소유 문서 판정. `prefers-reduced-motion: reduce` 에뮬레이션 시 다이얼로그·토스트에 move/scale 이 남지 않는지만 확인 | 종속 |
...
| IA-13 | IA | 직접 | **F3b 에서 뒤집혔다 — 이제 소비한다.** `TemplateListPage.tsx:124` 가 `useListState({ filterDefaults: FILTER_DEFAULTS })` 를 쓴다(`FILTER_DEFAULTS = { channel: 'all' }`). 채널 필터·검색어의 **단일 원천이 URL 쿼리스트링**이다: `useListState.ts:87` `useSearchParams` → `:89-91` page/`?q=`/sort → `:93-99` 필터. 갱신은 `patchParams` 한 통로(`:108-129`)로만 나가고 **`replace: true`**(`:125`)라 히스토리가 쌓이지 않는다. 기본값과 같은 값은 URL 에서 지운다(`:116`). 손으로 고친 `?channel=거짓말` 은 공용 `parseFilter`(`TemplateListPage.tsx:126-130`)가 허용 목록으로 걸러 '전체'로 되돌린다(그 롤아웃으로 `_shared/messaging.ts:37-41` 의 `parseMessageChannel` 사본이 소비자 0 이 되어 삭제됐다 — 주석이 그 경위를 남긴다). `useListState` 소비 화면은 **37곳**이며 **마케팅 6화면 전부**가 여기 든다. page/sort 표면은 여전히 없다(IA-04 gap · FS-036 §7 #4) | '알림톡' 필터 + '주문' 검색 → URL 이 **`/marketing/templates?channel=alimtalk&q=주문`**. 행을 열어 수정 폼 진입 후 Back → **그 조건 그대로** ✓. URL 을 새 탭에 복사 → 같은 view ✓. '전체 채널'로 되돌리면 `?channel=` 이 사라진다 ✓ | **pass** |
...
| 2 | ~~IA-13~~ | ~~P0~~ | **해소 (F3b · 2026-07-17)** — `TemplateListPage.tsx:124` 가 공용 `useListState` 를 소비해 `?channel=`·`?q=` 를 URL 이 소유한다(`useListState.ts:87-99,125`). F5·Back·링크 공유로 복원된다. `useListState` 소비 화면은 이제 **37곳**(마케팅 6화면 전부 포함) | — | **닫힘** |
...
- [x] **브리핑의 사실 주장을 그대로 쓰지 않고 재확인했다** — F2 시점에는 `useListState`/`useDebouncedSearch` 소비자가 `MembersPage`/`MembersToolbar` 2파일뿐이었다. **2026-07-17 재확인 결과 F3b 롤아웃으로 `useListState` 소비 화면이 37곳이 됐고 이 화면(`TemplateListPage.tsx:124`)도 그중 하나다** — 그래서 COMP-10·IA-13 을 pass 로 뒤집었다
