...
| 판정 기준일 | **2026-07-17 · `HEAD = 4b805ad`** (F3a·F3b·통합 머지 후 — PR #8·#11·#12·#14·#16). 이전 판정은 F2(`3cd3078`) 기준이었고 **COMP-10 · IA-13 이 pass 로 뒤집혔다**. **A11Y-11 · ERP-09 는 gap 그대로다** — 전자는 `SegmentPicker`·수신거부 토글이 `FormField` 를 거치지 않아 F3a 의 `aria-required` 주입이 닿지 않고, 후자는 예약 시각 해석이 여전히 브라우저 로컬이다(§2 · §3 · §5). **E2E 미실행 — 판정 근거는 전부 코드 대조다** |
...
> **`상속` 인데 `gap` 인 경우**: 표기 규약상 `상속`은 보통 `종속`으로 끝난다. 그러나 **이 화면이 렌더하는 바로 그 표면에서 DS 계약이 깨져 있음을 코드로 확인한 경우**에는 `gap` 으로 적고 §5 에 **범위=앱 전역(DS)** 으로 이관한다 — 소유가 DS 라는 사실이 미충족을 pass 로 만들지는 않기 때문이다. 이 문서에서는 MOTION-01/02/03 이 그렇다.
...
| TOKEN-03 | TOKEN | 상속 | 이 화면이 렌더하는 애니메이션 표면은 DS 소유뿐이다 — Toast(삭제·저장 성공)와 ToggleSwitch transition(`ToggleSwitch.css:32,56`). easing 토큰을 이 화면 코드가 직접 참조하는 곳은 0건이다 | tokens codegen · `Toast.css` 소유 문서의 판정을 따른다 | 종속 |
...
| A11Y-11 | A11Y | **직접** | **부분 충족.** 짝 없는 `aria-invalid` 는 0건이다 — 발송명(`EmailFormPage.tsx:275-276`)·제목(`:295-298`)·예약 일시(`:389-392`)가 `aria-invalid` + `aria-describedby={errorIdOf(...)}` 를 함께 싣고, 본문은 `TextareaField` 가 자동 배선하며, 발신자는 `SelectField isInvalid`(`:312`)가 담당한다. **그러나 두 곳이 계약을 지키지 않는다**: ① `SegmentPicker` — required 표식이 `aria-hidden` `<span>`(`SegmentPicker.tsx:109-113`), 그룹·체크박스에 `aria-required` 없음, 오류 `<p role="alert">`(`:137-140`)가 어느 컨트롤과도 연결되지 않음. ② **수신거부 토글** — 오류 `<p role="alert">`(`EmailBodyCard.tsx:117-120`)가 `ToggleSwitch` 와 `aria-describedby` 로 연결되지 않는다(DS 가 그 prop 을 노출하지 않는다 — `ToggleSwitch.tsx:19-38`). **이메일은 SMS 보다 한 곳 더 걸린다** — 수신거부는 이 화면에만 있는 검증 대상 토글이다 | ① `grep -rn "aria-invalid" apps/admin/src/pages/marketing/email` → 4건 전부 `aria-describedby` 동반(pass). ② 세그먼트 미선택 + 수신거부 끄고 제출 → 오류 2건이 뜨지만 RTL 로 체크박스·토글의 `aria-describedby` 를 조회하면 둘 다 `null`, `aria-required` 도 없다 → **요구의 'required 는 native required/aria-required 로 노출'·'aria-invalid 시 error id 연결' 절 미충족** | **gap** |
...
| MOTION-01 | MOTION | 상속 | **충족하지 않는다.** 이 화면이 Modal 표면을 렌더한다(삭제 확인 · 일괄 삭제 · 충돌 다이얼로그 · 미저장 가드). 그러나 **Motion 라이브러리가 리포에 도입되지 않았다** — `motion`/`framer-motion` 이 어느 `package.json` 에도 없고 `Modal.tsx` 에 `AnimatePresence`/`motion.` 참조가 0건이다. backdrop fade·dialog scale·exit 후 unmount 가 전부 없이 즉시 pop in/out 한다 | `grep -rn "AnimatePresence\|motion\." packages/ui/src/organisms/Modal/Modal.tsx` → 0건. `grep -rn "framer-motion\|\"motion\"" **/package.json` → 0건. 삭제 확인을 열고 닫으면 전환 없이 즉시 나타났다 사라진다 | **gap** (범위: 앱 전역 · DS 소유) |
| MOTION-02 | MOTION | 상속 | **충족하지 않는다.** 이 화면이 토스트를 띄운다(삭제·저장 성공). `ToastProvider` 에 `AnimatePresence` 가 0건이라 dismiss 가 즉시 filter-out 으로 끊긴다 — entrance 만 CSS 로 있고 exit 애니메이션이 없다 | `grep -rn "AnimatePresence" apps/admin/src/shared/ui/ToastProvider.tsx` → 0건. 이메일 발송을 저장 → 성공 토스트가 자동 소멸할 때 fade/translate 없이 사라진다 | **gap** (범위: 앱 전역 · DS 소유) |
| MOTION-03 | MOTION | 상속 | **충족하지 않는다.** 이 화면이 `ToggleSwitch` 를 **2개** 렌더한다 — 광고성 여부(`EmailFormPage.tsx:340-349`) · **수신거부 링크**(`EmailBodyCard.tsx:104-116`). `ToggleSwitch.css` 가 `transition: background-color`(`:32`)와 **`transition: transform`(`:56`)** 를 걸면서 `@media (prefers-reduced-motion: reduce)` off 를 두지 않는다 — quality-bar 가 MOTION-03 본문에서 직접 지목한 위반이 그대로 남아 있다. 전역 reduced-motion 게이트도 없다(Motion 미도입) | `grep -n "prefers-reduced-motion" packages/ui/src/atoms/ToggleSwitch/ToggleSwitch.css` → **0건**. OS/DevTools 에서 `prefers-reduced-motion: reduce` 를 켜고 수신거부 토글을 조작 → handle 이 여전히 transform transition 으로 미끄러진다 | **gap** (범위: 앱 전역 · DS 소유) |
...
| `종속` | **10** | TOKEN-02 · TOKEN-03 · TOKEN-04 · TOKEN-05 · FEEDBACK-02 · A11Y-01 · A11Y-02 · IA-01 · EXC-01 · EXC-02 |
| `n-a` | **2** | FEEDBACK-06 · A11Y-12 |
| `gap` | **7** | A11Y-11 · MOTION-01 · MOTION-02 · MOTION-03 · IA-02 · IA-04 · EXC-03 |

**검산**: pass 11 + 종속 10 + n-a 2 + gap 7 = **30** ✓ (차원별 STATE 3 · TOKEN 5 · COMP 1 · FEEDBACK 3 · A11Y 4 · MOTION 3 · IA 5 · EXC 6 = **30** ✓)

> **2026-07-17 · `4b805ad` 기준 변경**: F2 판정의 gap 9건 중 **2건이 pass 로 뒤집혔다** — **COMP-10**(F3b 가 `useListState` → `useDebouncedSearch` 를 이 화면에 배선) · **IA-13**(같은 훅이 조회 상태를 URL 로 옮겼다). **이 화면에는 그 배선을 고정하는 회귀 테스트가 있다** — `EmailListPage.test.tsx:4` '여기서 증명하는 것은 훅이 아니라 **배선**이다'. **IA-02 · EXC-03 은 gap 을 유지하되 사유가 바뀌었다**(브랜치 폴백 ✕ → h1 이중·행위 미반영 / 소비자 0 ✕ → 7곳 있으나 marketing 미배선). **A11Y-11 은 gap 그대로다** — F3a 가 `FormField.tsx:50-56` 에 `aria-required` 런타임 주입을 넣었지만 **`SegmentPicker`(`_shared/SegmentPicker.tsx:109-112`)와 수신거부 토글은 `FormField` 를 거치지 않아** 그 주입이 닿지 않는다. **MOTION-01/02/03 도 그대로다**(Motion 라이브러리 미도입).
>
> **⚠ P0 gap 7건 — quality-bar §How to use 기준 '배치 실패' 사유다.** **NFR-034(SMS)와 판정이 여전히 완전히 동일하다** — 같은 프레임워크·같은 `_shared` 를 쓰는 형제 화면이므로 뒤집힌 것도 남은 것도 형제다. 성격이 갈린다: **이 화면/섹션 소유 4건**(A11Y-11 — `_shared/SegmentPicker.tsx` · IA-02 · IA-04 · EXC-03)과 **앱 전역/DS 소유 3건**(MOTION-01/02/03). 후자는 이 화면만 고쳐서 닫을 수 없다.
...
| A11Y-16 | P1 | 이 화면의 신규 표면이 계약을 대체로 지킨다 — `EmailPreview` `aria-label`(`:102`) · `VariableInsertBar` 칩 `aria-label`(`:65`) + `tds-ui-focusable` · `ToggleSwitch` `role="switch"` · 수신거부 미포함 경고가 이중(비색상) 인코딩(`EmailPreview.tsx:118` — 배지 텍스트). **단 `SegmentPicker` 가 그룹 semantics 없이 `<ul>` + bare `<span>` 라벨이다** | axe 로 `/marketing/email/new` 스캔 | 부분 pass(→ §5 #3) |
...
| # | 요구 ID | P | 내용 | 범위 | 이관 |
|---|---|---|---|---|---|
| 1 | ~~**IA-13**~~ | ~~P0~~ | **해소 (F3b · 2026-07-17)** — `EmailListPage.tsx:116` 이 공용 `useListState` 를 소비해 `?status=`·`?q=` 를 URL 이 소유한다(`useListState.ts:87-99,125`). F5·Back·링크 공유로 복원되고 `EmailListPage.test.tsx` 가 그 배선을 고정한다. `useListState` 소비 화면은 이제 **37곳**(마케팅 6화면 전부 포함) | — | **닫힘** |
...
| 3 | **A11Y-11** | **P0** | **gap 유지 (2026-07-17 재확인).** `SegmentPicker` 의 required 표식이 `aria-hidden` `<span>` 이고(`_shared/SegmentPicker.tsx:109-112`) `aria-required` 가 없으며, 체크박스 그룹에 `fieldset`/`legend`·`role="group"` 도 없고, 오류 `<p role="alert">`(`:137-140`)가 체크박스와 `aria-describedby` 로 연결되지 않는다. 수신거부 토글도 같은 결(`EmailBodyCard.tsx:102-124` — bare `<span>` 라벨 + `ToggleSwitch` 가 `aria-describedby` prop 미노출). **⚠ F3a 가 이 구멍을 덮지 못한다** — `FormField.tsx:50-56` `withAriaRequired` 의 런타임 `cloneElement` 주입은 `FormField` 를 **거치는** 자식(`input`/`select`/`textarea`/`SelectField`)에만 적용되는데 이 둘은 `FormField` 를 아예 쓰지 않는다. **이 폼의 나머지 required 필드는 전부 주입된다**(발송명 `<input>` `EmailFormPage.tsx:267` · 제목 `<input>` `:287` · 발신자 `<SelectField>` `:309` · 발송 방식 `<SelectField>` `:371` · 예약 일시 `<input>` `:383`; 본문은 `TextareaField.tsx:64-65` 가 자기 것을 낸다) — 결함은 **`_shared/SegmentPicker.tsx` 한 곳**으로 국소화됐다 | **`_shared/SegmentPicker.tsx` 1곳**(SMS·이메일·뉴스레터 3화면 공유) + DS(`ToggleSwitch`) | A11 · A40 (FS-035 §7 #3) |
| 4 | **IA-02** | **P0** | **gap 유지 · 사유 전환.** ~~브랜치 라벨 폴백~~은 통합의 `findCoveringLeaf`(`nav-config.ts:260-278,297-299`)로 **해소**됐다 — AppHeader h1 은 이제 '이메일 발송' 이다. 남은 것: **폼 라우트에 `<h1>` 이 둘**(`AppHeader.tsx:101` + `EmailFormPage.tsx:252`)이고, `nav-config.ts:294-296` 이 '등록/수정' 행위를 제목에 넣지 않는 것을 의도로 못 박아 `/new` 와 `/:id/edit` 의 상단 h1 이 동일하다. **목록 라우트는 h1 1개 — 그 축은 pass** | 앱 전역(모든 하위 라우트) | A40 · A11 (FS-035 §7 #1) |
| 5 | **IA-04** | **P0** | Pagination 이 없다 — `marketing/**` 전역 `Pagination`·`pageSize` 참조 0건. DS `Pagination` 은 F3a 에서 범위/page-size 를 **opt-in**(`Pagination.tsx:112` `pageSize > 0`)으로 열었으나 실제 opt-in 소비자는 `stats/*`·`logs` 뿐이고, `useListState` 가 주는 `page`·`clampPage`(`useListState.ts:89,217-223`)도 이 화면이 쓰지 않는다 | 이 화면 + `CrudListShell` | A11 (FS-035 §7 #12) |
| 6 | **EXC-03** | **P0** | **gap 유지 · 범위 축소.** 쓰기 권한 게이팅이 없다 — 등록·수정·삭제 버튼이 권한과 무관하게 렌더. **`useRouteWritePermissions`(`RequirePermission.tsx:45`) 소비처는 0 → 7곳**(products 3 · settings 4)이 됐다. 훅·선례는 완비돼 있고 **`pages/marketing/**` 이 아직 배선되지 않았을 뿐**이다(grep = 0) | **이 섹션**(앱 전역 아님 — 선례 7곳 존재) | A11 · A63 (FS-035 §7 #13 · BE-035) |
| 7 | **MOTION-01** | **P0** | Modal enter/exit 모션 없음 — Motion 라이브러리 미도입 | **앱 전역 · DS 소유** | A40 (DS) |
| 8 | **MOTION-02** | **P0** | Toast exit 모션 없음 — `ToastProvider` 에 `AnimatePresence` 0건 | **앱 전역 · DS 소유** | A40 (DS) |
| 9 | **MOTION-03** | **P0** | `ToggleSwitch.css:32,56` 의 transition 에 `prefers-reduced-motion` off 없음. 이 화면이 ToggleSwitch 2개(광고성·수신거부)를 렌더 | **앱 전역 · DS 소유** | A40 (DS) |
| 10 | COMP-12 | P2 | **본문은 pass**(카운터·`maxLength`·검증이 모두 글자수라 일치 — SMS 와 갈리는 지점). **제목(120자)에 카운터가 없다** — `<input>` 이라 조용히 멈춘다(`EmailFormPage.tsx:287-300`). counting 기준(code point)이 문서에 정의되지 않아 서버 강제와 맞출 근거가 없었다(BE-035 §4 EP-03 이 이제 못 박는다) | 이 화면 | A11 (FS-035 §7 #5) |
| 11 | EXC-18 / EXC-10 | P1 | 일괄 삭제에 Shift-range · 강화 confirm · 진행률 · 실행 중 취소 · 실패 항목 식별이 없다(건수만) | 이 화면 + `useCrudList` | A11 (FS-035 §7 #12) |
| 12 | IA-10 | P2 | 2-col preview shell 이 손조립이고 `SmsFormPage` 와 **문자 그대로 같은 상수**를 각자 선언한다(`EmailFormPage.tsx:82-87` ≡ `SmsFormPage.tsx:88-93`) | marketing(+popup/banner/quote) | A11 |
| 13 | ERP-09 | P2 | **gap 유지 (2026-07-17 재확인).** 과거 예약 판정 `isPastLocal` 이 브라우저 로컬 TZ(`validation.ts:17-21`)이고 `scheduledAt` 이 오프셋 없는 `datetime-local` 문자열이다 — 서버 해석 TZ 가 계약에 없다. **⚠ F3b 의 TZ 수렴이 이 축을 덮지 못했고 불일치가 선명해졌다** — `shared/format.ts:63,76-85,171-175` 가 **표시**를 KST 로 고정해 목록의 예약 시각은 KST 로 찍히는데 **검증은 로컬**이다. 같은 값에 두 기준이 걸린다. 같은 결함이 SMS·뉴스레터에도 있다(`isPastLocal` 사본 3벌). **더불어 야간 광고 제한 자체가 이메일에 없고 그 근거가 어디에도 없다**(`validation.ts:85-105` — `isNightAt` 0건) | 이 화면 + `_shared` | **A01 · A63** (FS-035 §7 #7 · BE-035 §7.5) |
| 14 | STATE-05 | P1 | 진짜 비어있음 분기에 생성 CTA 가 없다 — `empty.createAction` 미전달(`EmailListPage.tsx:159-164`) | 이 화면 | A11 |
| 15 | A11Y-13 / EXC-07 | P1 | `segmentIds`·`includeUnsubscribe` 는 `register` 를 거치지 않은 `setValue` 전용 필드라 **ref 가 없어 `setFocus` 가 동작하지 않는다**(`EmailFormPage.tsx:332-334` · `EmailBodyCard.tsx:106-111`) — 그 둘만 위반하면 오류는 뜨지만 포커스가 이동하지 않는다 | 이 화면 + `_shared` | A11 |
| 16 | EXC-05 / EXC-11 / EXC-19 | P1 | 프론트 타임아웃 상한 · 오프라인 감지 · 세션 만료 시 폼 draft 보존이 없다. **본문 5,000자를 모으는 폼이라 손실이 크다** | 앱 전역 | A11 · A40 (FS-035 §7 #17) |
| 17 | IA-14 | P1 | 반응형 계약이 선언되지 않았다. 제목 컬럼 때문에 표가 SMS 보다 넓다 | 앱 전역 | A40 |
| 18 | (기능) | — | **발송 트리거 UI 가 없다** — `/send` 심(`data-source.ts:70`)에 호출부 0건. **SMS(`sms/data-source.ts:71`)와 대칭**이다. 구현 시 **멱등키 필수**(BE-035 §7.3). 발송 UI 가 없으므로 **중복 발송 경합은 현재 성립하지 않는다** — 트리거가 붙는 순간 EXC-08 의 진짜 대상이 된다. **테스트 발송('내게 보내보기')도 없다** | 이 화면 | A11 · A63 (FS-035 §7 #6 · BE-035 §7.11 #1) |
| 19 | (보안) | — | **수신거부가 불리언뿐이다** — URL·토큰이 도메인 모델에 없고 미리보기의 '수신거부'는 링크가 아니다(`EmailPreview.tsx:113-116`). 서버가 수신자별 서명 토큰을 주입해야 한다. 또한 `includeUnsubscribe` 를 광고성 여부와 무관하게 강제하는 현재 규칙(`validation.ts:74-84`)이 법 요구보다 강하다 | 이 화면 + 서버 | A63 · A01 (FS-035 §7 #10 · BE-035 §7.7) |
| 20 | (보안) | — | 본문이 평문이라 관리자 화면에서 XSS 는 성립하지 않는다(`dangerouslySetInnerHTML` 0건). **그러나 수신자 메일 클라이언트가 정본이므로 서버 정제가 지금도 필요하고**, HTML 에디터가 오면 즉시 필수가 된다. **제목의 헤더 인젝션(`\r`·`\n`) 방어가 프론트·서버 어디에도 없다** | 이 화면 + 서버 | A63 (FS-035 §7 #11 · BE-035 §7.8) |
| 21 | (감사) | — | 발송 감사 필드(작성자·실행자·실행 시각·동의 근거)가 도메인 타입에 없고, 발송 이력이 hard-delete 로 함께 사라진다. **오픈/클릭 이벤트가 발송 후에도 도착하므로 이메일은 soft-delete 근거가 더 강하다**(§4.3) | 이 화면 + 서버 | A63 (BE-035 §7.9) |
...
| `useListState.test.tsx` · `useDebouncedSearch.test.tsx` | **이 화면이 쓰지 않는 훅의 테스트** — IA-13·COMP-10 gap 이 닫히면 이 화면에도 적용된다 |
| grep 판정 | `Pagination`(IA-04) · `useListState`/`useDebouncedSearch`(IA-13·COMP-10) · `useRouteWritePermissions`(EXC-03) · `AnimatePresence`/`framer-motion`(MOTION-01/02) · `prefers-reduced-motion`(MOTION-03) · `isNightAt`(ERP-09) · `dangerouslySetInnerHTML`(XSS) · hex/px/border-keyword(TOKEN-01) |
...
- [x] §2.1 산수를 검산했다 — pass 11 + 종속 10 + n-a 2 + gap 7 = **30** ✓ (차원별 STATE 3 · TOKEN 5 · COMP 1 · FEEDBACK 3 · A11Y 4 · MOTION 3 · IA 5 · EXC 6 = **30** ✓)
- [x] **2026-07-17 · `HEAD = 4b805ad`(F3a·F3b·통합) 기준으로 30행 전수 재검증했다. F2(`3cd3078`) 판정을 재사용하지 않았다.** 뒤집힌 판정 2건: **COMP-10** gap→pass(`EmailListPage.tsx:116,145-152` → `useListState.ts:227-230` → `useDebouncedSearch.ts:87,93-95,121-129`; `EmailListPage.test.tsx` 가 배선을 고정) · **IA-13** gap→pass(`useListState.ts:87-99,108-129`). **사유가 바뀐 gap 2건**: IA-02(브랜치 폴백 → h1 이중·행위 미반영) · EXC-03(소비자 0 → 7곳 있으나 marketing 미배선). **유지된 gap 5건**: A11Y-11(`SegmentPicker`·수신거부 토글 — F3a 주입이 닿지 않음을 코드로 확인) · MOTION-01/02/03(Motion 미도입) · IA-04. **ERP-09 도 gap 유지** — F3b 의 KST 표시 고정이 예약 시각 **해석**(`validation.ts:17-21` `isPastLocal`)을 덮지 못했다. **grep 함정 2개를 피했다**: ① `aria-required` 는 런타임 `cloneElement` 주입이라 앱 소스 grep 으로 판정하지 않았다 ② `useDebouncedSearch` 는 `useListState` 경유 소비라 화면 파일 grep 이 0건이어도 pass 다
- [x] `상속`인데 `gap`인 3건(MOTION-01/02/03)의 표기 근거를 §1.1 각주로 밝히고 §5 에 **범위=앱 전역·DS 소유**로 이관했다
...
- [x] **NFR-034(SMS)와 갈리는 지점을 명시했다** — COMP-12(이메일 pass · SMS gap) · ERP-09(야간 판정 부재) · §4.3 이메일 고유 3항목 · A11Y-11 이 한 곳 더 걸림(수신거부 토글)
