...
| TOKEN-03 | TOKEN | 상속 | easing 토큰 소비 표면: 스켈레톤 펄스(`tds-ui-skeleton` — `CrudTable.tsx:148` · `FormPageShell.tsx:173`) · Toast(토글·저장 결과) · DS `<Button>` transition · **`ToggleSwitch` 의 track/knob transition**(`ToggleSwitch.css:32,56` — `var(--tds-motion-duration-fast)`). **이 화면이 animation/transition 을 직접 선언하지 않는다**(grep 0건) | tokens codegen · `Toast.css` · `ToggleSwitch.css` 판정에 종속 | **종속** |
...
| MOTION-01 | MOTION | 상속 | Modal 표면 4종(삭제 ×2 · 이탈 가드 · 409 충돌). enter/exit transition 은 DS `Modal` organism 이 소유한다 — 이 화면은 애니메이션을 선언하지 않는다 | DS Modal 판정에 종속. (참고: `packages/ui/src`·`apps/admin/src` 에 Motion/AnimatePresence 소비가 **0건**이므로 소유 문서에서 gap 일 가능성이 높다 — **그 판정은 이 문서의 몫이 아니다**) | **종속** |
| MOTION-02 | MOTION | 상속 | Toast 표면 3종(저장 · 삭제 · **토글**). exit 애니메이션은 `ToastProvider`/`Toast` 가 소유한다 | DS Toast 판정에 종속. (참고: `Toast.css:110` 에 reduced-motion 블록이 있으나 Motion 라이브러리 미도입 — 소유 문서 판정) | **종속** |
| MOTION-03 | MOTION | 상속 | reduced-motion 게이트가 걸릴 이 화면의 표면: 스켈레톤 펄스(`tds-ui-skeleton` — `ui.css:110` 에 reduced-motion 블록 **있음**) · Toast(`Toast.css:110` **있음**) · Modal · DS Button(`Button.css:158` **있음**) · **`ToggleSwitch`**. **이 화면이 transform/transition 을 직접 선언하지 않는다**(grep 0건). **⚠ 소유 문서에 실재 gap 이 있다**: `ToggleSwitch.css:32` `transition: background-color …` · **`:56` `transition: transform …`** 에 **`@media (prefers-reduced-motion: reduce)` 블록이 없다**(grep 확인 — `packages/ui/src/atoms/ToggleSwitch/ToggleSwitch.css` 는 reduced-motion 히트 0건). quality-bar MOTION-03 이 **`ToggleSwitch.css` 를 appliesTo 에 명시 지목하고 '누락된 off 를 추가한다'를 요구 본문에 적어 둔** 바로 그 항목이다. **ToggleSwitch 는 이 화면에만 있다**(형제 두 화면엔 없다) — 즉 알림 관리 3화면 중 **이 화면이 그 gap 에 노출되는 유일한 화면**이다. 그러나 해소는 `packages/ui` 계약이며 **화면 코드로는 불가능**하다 | 전역 motion config·`ToggleSwitch.css` 판정에 종속. 재현: `prefers-reduced-motion: reduce` 를 켜고 자동 발송 토글 — **knob 이 여전히 미끄러지면 DS gap**(화면 gap 아님). 이 화면에서는 로컬 transition 선언이 0건임만 확인 | **종속** |
...
| **템플릿 삭제로 규칙 무력화** | 삭제 전 경고 또는 서버 거절 | **미충족 — 조용히 깨진다.** `rulesUsingTemplate`(`store.ts:383-385`)이 그 용도로 존재하나 **프로덕션 소비처 0건**(grep). 발송 규칙 목록에 가야만 danger 배지가 보인다(BE-064 §7.5) |
...
| 규칙이 가리키는 템플릿이 그 이벤트의 것이다 | **부분 충족.** select 후보를 채널+트리거로 좁힌다(`store.ts:366-374`). **서버 재검증이 없으면 미치환 변수가 빈칸으로 발송된다**(BE-064 §7.4 【보안·정합 판정】) |
...
| 20 | (MOTION-03 · DS) | P0 | **`ToggleSwitch.css:32,56` 의 transition 에 reduced-motion off 가 없다**(grep 확인) — quality-bar MOTION-03 이 명시 지목한 항목. **ToggleSwitch 는 알림 관리 3화면 중 이 화면에만 있다.** 판정은 `종속`(DS 소유)이나 노출 화면으로서 기록한다 | `packages/ui` | A41 / DS |
...
**그 밖의 도구**: `grep`(TOKEN-01 · COMP-01 · COMP-03 · EXC-04③ · EXC-05 · EXC-08 · EXC-11 · ERP-13 · MOTION-03 판정) · RTL(A11Y-11 의 describedby↔alert id 일치, A11Y-12) · 기존 테스트 — `_shared/notification.test.ts`(트리거·변수·광고성·바이트·필터/검색/정렬·중복 규칙 순수 규칙 회귀 · 213줄) · `_shared/store.test.ts`(템플릿 후보·템플릿명·`rulesUsingTemplate`) · `send/send.test.ts`(스키마 회귀 — 템플릿 필수 · 중복 금지 · selfId 제외) · **`send/RuleListPage.test.tsx`(jsdom 스모크 — 픽스처 행 렌더 · 이벤트 열 · **마케팅 링크 href** · ON/OFF 스위치 · **A11Y-12 aria-pressed**). E2E 는 이번 배치에서 금지라 jsdom 에서 실제 provider 로 렌더한다(`RuleListPage.test.tsx:8`)**.
